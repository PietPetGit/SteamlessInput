...
_SYSTEM_ACTIONS = ("alt_tab", "force_kill", "power_off", "toggle_magnifier",
                   ...
                   "gamepad_mode_toggle", "big_picture", "escape_elevated")
...
VMENU_CUSTOM_EXTS = (".png", ".jpg", ".jpeg", ".gif", ".bmp", ".webp",
                     ".tiff", ".tif", ".ico")
# --- Program icons ----------------------------------------------------------
# The Custom tab can also take a PROGRAM (or a shortcut to one) and lift the
# icon Explorer shows for it, so binding "launch Firefox" to a menu button
# doesn't mean hunting down a PNG of the Firefox logo first. These are the
# things Windows resolves an icon for; the extracted image then goes through
# the very same normalize/store path an uploaded image does.
VMENU_PROGRAM_EXTS = (".exe", ".lnk", ".url", ".ico", ".dll", ".cpl", ".msc",
                      ".bat", ".cmd", ".com", ".appref-ms", ".msi", ".scr")
# The size we ask the shell for. 256 is the largest icon Windows stores, and
# anything bigger just upscales  VMENU_CUSTOM_ICON_PX then takes it down.
...
def _store_custom_vmenu_icon(img, hash_src):
    ...
    # transparent pixels, which some ICOs/BMPs carry). A fully-opaque image has
    # no transparent border, so this is a harmless no-op there.
...
def _win_hbitmap_to_image(hbm):
    """A GDI HBITMAP → RGBA PIL image (or None). Always reads it back as
    top-down 32bpp BGRA, which is what both icon paths below hand us."""
    import ctypes
    ...
    if not gdi32.GetObjectW(hbm, ctypes.sizeof(BITMAP), ctypes.byref(bm)):
        return None
    w, h = int(bm.bmWidth), int(abs(bm.bmHeight))
    if w <= 0 or h <= 0 or w > VMENU_CUSTOM_MAX_DIM or h > VMENU_CUSTOM_MAX_DIM:
        return None
    bi = BITMAPINFO()
    ...
    if not hdc:
        return None
    try:
        got = gdi32.GetDIBits(hdc, hbm, 0, h, buf, ctypes.byref(bi), 0)
    finally:
...
def _win_shell_icon_image(path, px):
    """The icon the shell itself would draw for `path`, at up to `px` square,
    as an RGBA PIL image (or None).

    Uses IShellItemImageFactory, which is what Explorer uses: it resolves .lnk
    shortcuts to their target, honors a shortcut's custom icon, and returns the
    largest icon a program actually ships (256px where present) rather than the
    32px system-list version."""
    import ctypes
    from ctypes import wintypes
    ...
    iid = GUID(0xbcc18b79, 0xba16, 0x442f,
               (ctypes.c_ubyte * 8)(0x80, 0xc4, 0x8a, 0x59,
                                    0xc3, 0x0c, 0x46, 0x3b))
    ole32.CoInitializeEx.argtypes = [ctypes.c_void_p, wintypes.DWORD]
    ole32.CoInitializeEx.restype = ctypes.c_long
    # The Tk thread may already be in an apartment (RPC_E_CHANGED_MODE)  that's
    # fine, we just must not uninitialize someone else's.
    hr = ole32.CoInitializeEx(None, 0x2)       # COINIT_APARTMENTTHREADED
    mine = hr in (0, 1)                        # S_OK / S_FALSE
    try:
        ...
        if shell32.SHCreateItemFromParsingName(
                path, None, ctypes.byref(iid), ctypes.byref(ptr)) < 0:
            return None
        if not ptr:
            return None
        vtbl = ctypes.cast(
        ...
        try:
            # vtable: [0] QueryInterface [1] AddRef [2] Release [3] GetImage
            ...
            # ICONONLY: never substitute a document thumbnail for the program's
            # own icon. BIGGERSIZEOK lets it hand back the native 256 instead of
            # a blurry upscale when we asked for something in between.
            flags = 0x4 | 0x1
            if get_image(ptr, SIZE(px, px), flags, ctypes.byref(hbm)) < 0:
                return None
            if not hbm:
                return None
            try:
                return _win_hbitmap_to_image(hbm)
            finally:
...
def _win_extract_icon_image(path):
    """Fallback icon read for `path` via ExtractIconExW  the classic API, for
    the odd file the shell factory refuses. Returns an RGBA PIL image or None.

    Icons that predate 32bpp alpha carry their transparency in a 1bpp MASK
    bitmap instead, so rebuild alpha from that when the color bitmap has none
    (otherwise those import as a fully-transparent, invisible tile)."""
    import ctypes
    from ctypes import wintypes
    shell32 = ctypes.WinDLL("shell32")
    ...
    big = wintypes.HICON()
    if shell32.ExtractIconExW(path, 0, ctypes.byref(big), None, 1) < 1:
        return None
    if not big:
        return None
    try:
        info = ICONINFO()
        if not user32.GetIconInfo(big, ctypes.byref(info)):
            return None
        try:
            if not info.hbmColor:
                return None
            img = _win_hbitmap_to_image(info.hbmColor)
            if img is None:
                return None
            if not img.getchannel("A").getbbox():
                mask = _win_hbitmap_to_image(info.hbmMask)
                if mask is not None and mask.size == img.size:
                    # Mask is white where the icon is transparent.
                    img.putalpha(mask.convert("L").point(
                        lambda v: 0 if v > 127 else 255))
                else:
                    img.putalpha(255)
            return img
        finally:
...
def extract_program_icon_image(path):
    """The icon a program / shortcut / icon-file shows in Explorer, as an RGBA
    PIL image. Raises ValueError with a user-facing message if there is none.

    Shortcuts resolve to whatever icon they point at, so picking a Start-menu
    entry Just Works."""
    import os
    import sys
    if not path or not os.path.exists(path):
        raise ValueError("That file could not be found.")
    if sys.platform != "win32":
        raise ValueError("Program icons aren't available on this system.")
    path = os.path.abspath(path)
    blank = None            # a read that worked but came back fully transparent
    for fn in (lambda: _win_shell_icon_image(path, VMENU_PROGRAM_ICON_PX),
               lambda: _win_extract_icon_image(path)):
        try:
            img = fn()
        except Exception:
            img = None
        if img is None:
            continue
        if img.getchannel("A").getbbox():
            return img
        blank = blank or img
    if blank is not None:
        # Every path read pixels but none carried alpha (a pre-alpha icon whose
        # mask we couldn't pair up)  take it as fully opaque rather than
        # importing an invisible tile.
        blank.putalpha(255)
        return blank
    raise ValueError("That program has no icon we could read.")


def _win_resolve_lnk_target(path):
    """A .lnk shortcut's stored target path and argument string, as
    (target, args)  either may be "" if the shortcut doesn't carry it. None
    on any COM failure (a corrupt or unreadable link).

    Uses IShellLinkW/IPersistFile (the same interfaces Explorer's own
    "Properties > Shortcut" tab reads), so it reports exactly what the
    shortcut is configured to run  not what the target happens to resolve to
    right now. SLGP_RAWPATH asks GetPath for the literal stored path with no
    filesystem verification/searching, so a shortcut to a temporarily-missing
    target still resolves instead of popping a "browse for target" dialog."""
    import ctypes
    from ctypes import wintypes
    ole32 = ctypes.WinDLL("ole32")
    ...
    try:
        ...
        psl = ctypes.c_void_p()
        hr = ole32.CoCreateInstance(ctypes.byref(clsid_shelllink), None, 0x1,
                                    ctypes.byref(iid_ishelllinkw),
                                    ctypes.byref(psl))
        if hr < 0 or not psl:
            return None
        vtbl_sl = ctypes.cast(
            psl, ctypes.POINTER(ctypes.POINTER(ctypes.c_void_p))).contents
        try:
            query = ctypes.WINFUNCTYPE(
                ctypes.c_long, ctypes.c_void_p, ctypes.POINTER(GUID),
                ctypes.POINTER(ctypes.c_void_p))(vtbl_sl[0])
            ppf = ctypes.c_void_p()
            if (query(psl, ctypes.byref(iid_ipersistfile),
                      ctypes.byref(ppf)) < 0 or not ppf):
                return None
            vtbl_pf = ctypes.cast(
                ppf, ctypes.POINTER(ctypes.POINTER(ctypes.c_void_p))).contents
            try:
                load = ctypes.WINFUNCTYPE(
                    ctypes.c_long, ctypes.c_void_p, wintypes.LPCWSTR,
                    wintypes.DWORD)(vtbl_pf[5])
                if load(ppf, path, 0) < 0:      # STGM_READ = 0
                    return None
                get_path = ctypes.WINFUNCTYPE(
                    ctypes.c_long, ctypes.c_void_p, wintypes.LPWSTR,
                    ctypes.c_int, ctypes.c_void_p, wintypes.DWORD)(vtbl_sl[3])
                get_args = ctypes.WINFUNCTYPE(
                    ctypes.c_long, ctypes.c_void_p, wintypes.LPWSTR,
                    ctypes.c_int)(vtbl_sl[10])
                buf = ctypes.create_unicode_buffer(32768)
                get_path(psl, buf, 32767, None, 0x4)   # SLGP_RAWPATH
                arg_buf = ctypes.create_unicode_buffer(32768)
                get_args(psl, arg_buf, 32767)
                return (buf.value, arg_buf.value)
            finally:
                release_pf = ctypes.WINFUNCTYPE(
                    ctypes.c_ulong, ctypes.c_void_p)(vtbl_pf[2])
                release_pf(ppf)
        finally:
            release_sl = ctypes.WINFUNCTYPE(
                ctypes.c_ulong, ctypes.c_void_p)(vtbl_sl[2])
            release_sl(psl)
    finally:
        if mine:
            ole32.CoUninitialize()
...
# Known per-user Windows dirs whose absolute (expanded) form gets rewritten
# back into an env-var TOKEN before a launch target is saved, longest/most-
# specific first so e.g. LOCALAPPDATA (nested inside USERPROFILE) matches
# before the broader USERPROFILE root would. Storing the token instead of
# today's username baked into an absolute path is what lets a saved config
# work on a different Windows account or a different PC  _launch_program()
# (tray.py) expands it back at the moment it actually runs, on whatever
# machine that turns out to be.
_WIN_ENV_TOKENS = ("LOCALAPPDATA", "APPDATA", "PROGRAMFILES(X86)",
                   "PROGRAMFILES", "USERPROFILE")


def _tokenize_windows_path(path):
    """An absolute path rewritten to start with a %ENV_VAR% token in place of
    its expanded per-user/per-machine root, if it lives under one of the
    known Windows env dirs above. Left unchanged otherwise (e.g. a path
    already under a fixed system location like C:\\Windows)."""
    import os
    norm = os.path.normpath(path)
    for name in _WIN_ENV_TOKENS:
        root = os.environ.get(name)
        if not root:
            continue
        root = os.path.normpath(root)
        if norm.lower().startswith(root.lower() + os.sep):
            return "%" + name + "%" + norm[len(root):]
    return path


def _discord_stable_launch_target(path):
    """Discord's own per-version exe (".../Discord/app-1.0.NNNN/Discord.exe")
    rewritten to the stable launcher indirection Discord's OWN Start Menu
    shortcut has used for years  Update.exe --processStart Discord.exe. The
    versioned app-N.N.NNNN folder gets renamed on every auto-update, so
    pinning the raw exe path breaks the button within weeks; Update.exe is
    the one path that never moves, on any PC Discord is installed on. Returns
    (target, args) or None if `path` isn't a Discord per-version exe."""
    import os
    if os.path.basename(path).lower() != "discord.exe":
        return None
    app_dir = os.path.dirname(path)
    if not os.path.basename(app_dir).lower().startswith("app-"):
        return None
    discord_dir = os.path.dirname(app_dir)
    if os.path.basename(discord_dir).lower() != "discord":
        return None
    updater = os.path.join(discord_dir, "Update.exe")
    if not os.path.isfile(updater):
        return None
    return (updater, "--processStart Discord.exe")


def _make_launch_target_portable(target, args):
    """Post-processes a resolved (target, args) pair so the saved launch
    action still works after moving the config to another PC/account, or
    after the app auto-updates: swap Discord's per-version exe for its stable
    Update.exe launcher, then rewrite whatever's left under a known per-user
    Windows dir (LOCALAPPDATA, APPDATA, ...)  which is where both Spotify
    and Discord install by default  to an env-var token."""
    stable = _discord_stable_launch_target(target)
    if stable is not None:
        target, args = stable
    return (_tokenize_windows_path(target), args)
...
# every time the button fires (see resolve_default_browser_exe / tray.py's
# _launch_program), not baked in at config time. Launched with no arguments,
# so it opens the browser's own start/new-tab page rather than any fixed
# site. Shared verbatim with the Linux tree so a config using it means the
...
# wherever Steam is ACTUALLY installed on this PC. The shipped default menu
# (default_virtual_menus) has two Steam buttons, and hardcoding
# "C:\Program Files (x86)\Steam\steam.exe" into art everyone downloads would
# leave both dead for anyone who put Steam on another drive  which is most
# people with a small system SSD. Unlike the browser sentinel this one KEEPS
# its stored args, because "-bigpicture" is the only thing separating the two
# buttons.
VMENU_LAUNCH_STEAM = "$STEAM$"


def resolve_steam_exe():
    """The Steam executable's real path on this machine, re-read on every call
    (see VMENU_LAUNCH_STEAM). Reads the per-user SteamPath the installer
    writes, falls back to the default install dir, and returns None when Steam
    isn't installed at all  the caller then simply does nothing rather than
    launching a path that doesn't exist."""
    import os
    import winreg
    for root, sub, name in (
        ...
        try:
            with winreg.OpenKey(root, sub) as k:
                d = os.path.normpath(str(winreg.QueryValueEx(k, name)[0]))
        except OSError:
            continue
...
def resolve_default_browser_exe():
    """The current OS default browser's executable path, re-read from the
    registry on every call  the same UserChoice/ProgId lookup Windows itself
    uses to decide what opens a link  so VMENU_LAUNCH_DEFAULT_BROWSER always
    reflects whatever's ACTUALLY default on THIS PC right now, not whichever
    browser happened to be default wherever the button was originally
    configured. None if it can't be resolved (no UserChoice set, or the
    ProgId's open command can't be read)."""
    import shlex
    import winreg
    try:
        with winreg.OpenKey(
                ...
                "UrlAssociations\\http\\UserChoice") as k:
...
def resolve_program_launch_target(path):
    ...
    button's cog-modal "Launch Program" hotkey run. For a .lnk shortcut this
    is its RESOLVED target and stored argument string (so it runs precisely
    what the shortcut was set up to run) rather than the .lnk file itself; for
    anything else the source IS the target, with no args. Either way the
    result is run through _make_launch_target_portable() so paths under the
    user's profile (Spotify, Discord, ...) are stored PORTABLY  as env-var
    tokens expanded at launch time  instead of hardcoding this one PC's
    username into the saved config."""
    import os
    if not path or not os.path.exists(path):
        return None
    path = os.path.abspath(path)
    if os.path.splitext(path)[1].lower() == ".lnk":
        try:
            got = _win_resolve_lnk_target(path)
        except Exception:
            got = None
        if got is not None and got[0]:
            # SLGP_RAWPATH hands back "%windir%\..." literally (unexpanded,
            # by design  it skips the filesystem work expansion needs) 
            # expand it ourselves first so _make_launch_target_portable sees
            # a real absolute path to match against the known env dirs.
            return _make_launch_target_portable(os.path.expandvars(got[0]),
                                                got[1])
        return _make_launch_target_portable(path, "")  # fall back to the .lnk
    return _make_launch_target_portable(path, "")
...
def set_custom_vmenu_icon_launch_info(icon, path, args):
    """Record the (path, args) a program/shortcut-sourced custom icon should
...
def import_program_vmenu_icon(src_path):
    """Lift a program's (or shortcut's) icon into the custom-icon dir and
    return its `custom:<name>` id  the picker's "use a program's icon" tile.
    ...
    the extracted PIXELS (not the source .exe, which may be hundreds of MB and
    changes on every update), so re-picking the same program  or picking both
    the exe and a shortcut to it  dedups onto one icon.

    Also resolves the source's launch target (a .lnk's real target+args, or
    the source path itself) and stashes it in a sidecar keyed by the new icon
    id, so the picker can auto-fill "Launch Program" once this icon is
    actually assigned to a button."""
...
#   - Steam / Big Picture     -> VMENU_LAUNCH_STEAM   (registry lookup)
#   - the browser button      -> VMENU_LAUNCH_DEFAULT_BROWSER
#   - Spotify / Discord       -> %APPDATA% / %LOCALAPPDATA% env tokens, the
#                                same portable form _make_launch_target_portable
#                                writes for a user's own picked program
#  because this list is shipped to every machine, and a path baked from THIS
# developer's PC (a username in it, Steam on C:) would be dead on most of them.
# A button whose program isn't installed simply does nothing when pressed; it
# is not an error, and the user can retarget or delete it in the editor.
#
...
_DEFAULT_VMENU_ENTRIES = (
    ...
    ("preset_spotify", "launch", r"%APPDATA%\Spotify\Spotify.exe", ""),
    # Discord's per-version app-1.0.NNNN folder is renamed on every auto-update,
    # so this is the stable Update.exe indirection Discord's own Start Menu
    # shortcut uses  the same rewrite _discord_stable_launch_target applies to
    # a user-picked Discord.exe.
    ("preset_discord", "launch", r"%LOCALAPPDATA%\Discord\Update.exe",
     "--processStart Discord.exe"),
    ("preset_notepad", "launch", r"%WINDIR%\System32\notepad.exe", ""),
    ("preset_browser", "launch", VMENU_LAUNCH_DEFAULT_BROWSER, ""),
    (None, None, None, None),          # free cell
    (None, None, None, None),          # free cell
    ("preset_shutdown", "script", "@echo off\nshutdown.exe /s /t 0\nexit", ""),
    ("preset_restart", "script", "@echo off\nshutdown.exe /r /t 0\nexit", ""),
    ("preset_lock", "script",
     "@echo off\nrundll32.exe user32.dll,LockWorkStation\nexit", ""),
    # Hibernate if the machine actually has a hiberfile, otherwise plain
    # suspend  SetSuspendState silently no-ops on a hibernate request when
    # hibernation is disabled, which is the default on a lot of laptops.
    ("preset_sleep", "script",
     "$state = if ((powercfg /a) -match 'Hibernate' -and "
     "(Get-ItemProperty 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Power' "
     "-Name HiberFileEnabled -ErrorAction SilentlyContinue)"
     ".HiberFileEnabled -eq 1) {\n    'Hibernate'\n} else {\n"
     "    'Suspend'\n}\n"
     "[System.Windows.Forms.Application]::SetSuspendState($state, $false, "
     "$false)", ""),
)
