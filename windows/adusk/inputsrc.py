...
def _sdl_pad_excluded(name):
    """SDL gamepads this source must NOT open:
      * Steam Controller / Steam Virtual Gamepad  the SC is owned by our
        custom hidapi driver (opening it in SDL too would double-drive it and
        fight over the HID handle). The STEAM DECK's built-in pad is NOT
        excluded  it's a normal SDL gamepad to us.
      * "Xbox 360 Controller"  our OWN ViGEm virtual pad, created in gamepad
        ...
        the name is safe."""
    n = (name or "").lower()
    return ("steam" in n and "deck" not in n) or "xbox 360" in n
...
class Sdl3GamepadSource:
    ...
    def __init__(self):
        ...
        # "gamepad" while gamepad mode feeds them to the virtual Xbox pad.
        # Set by the tray on mode changes; per-kind values live in
        # state.get_sdl_trigger_threshold.
        ...
        # jid -> stable identity (serial / vid:pid:name) so the tray can hand
        # the SAME virtual XInput device back to a pad that reconnects, and
        # (jid, uid, kind) drop events for the tray's dropout notice.
        self._pad_uids = {}
        self._drop_events = []
    ...
    def _rescan(self):
        ...
        for jid in list(self._pads):
            if jid not in ids:
                ...
                # still has to ask uid_of(jid) who left in order to park that
                # controller's virtual pad under an identity the reconnecting
                # controller will match. Pruned instead of popped.
                uid = self._pad_uids.get(jid)
                self._prune_pad_uids()
                # Publish the drop so the tray can park this controller's
                # virtual XInput device (and, for a guarded Nintendo pad, tell
                # the user once what just happened). Recorded for EVERY pad 
                # the tray decides what a given kind's dropout is worth.
        ...
        for jid in ids:
            if jid not in self._pads:
                ...
                if g:
                    ...
                    # and sensors off; force poll() to re-apply both.
                    self._home_led_applied = None
    ...
    def _pad_uid(self, g, jid, name):
        ...
        the tray hand the same virtual XInput device back after a dropout.
        Falls back to the name when a backend exposes no serial  two identical
        nameless pads then share a uid, which the tray's park bookkeeping
        handles by only ever letting ONE pad claim a parked device."""
        serial = None
        try:
    ...
    def uid_of(self, jid):
        ...
        point: it is what a dropped controller is parked under."""
    ...
    def _read_pad(self, g, kind=None):
        ...
        # the built-in default. Which family applies (OSK typing vs gamepad
        # mode) is the tray-set trigger_mode.
    ...
    def active_kind(self):
        ...
        jid = self._active_jid()
        if jid is None:
            # Fall back to the most-recently-active pad even outside the
            # freshness window, then to any connected pad.
            best_jid, best_t = None, 0.0
    ...
    def play_mode_rumble(self, on):
        ...
        so the timed gap between pulses doesn't block the input loop. Gated
        PER PAD by that controller kind's Haptics toggle (its Options
        category), so e.g. an Xbox pad can buzz while a muted Switch stays quiet."""
        targets = [(jid, g) for jid, g in list(self._pads.items())
                   if state.is_rumble_enabled(self._pad_kinds.get(jid, "switch"))]
        if not targets:
            return
        Thread(target=self._mode_rumble_worker,
               args=(bool(on), targets), daemon=True).start()

    def _mode_rumble_worker(self, on, targets):
        seq = ((self._RUMBLE_LIGHT, self._RUMBLE_STRONG) if on
               else (self._RUMBLE_STRONG, self._RUMBLE_LIGHT))
        for lo, hi, ms in seq:
            for jid, g in targets:
                # Not droppable: this two-pulse pattern IS the confirmation
                # that the mode flipped  the guard's pacing must not eat the
                # second pulse and turn "off" into "on".
                self._rumble_one(g, lo, hi, ms, jid=jid, droppable=False)
