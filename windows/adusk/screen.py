...
# Controller catalog for the per-kind Shift/Enter hint glyphs. Defensive so a
# stripped-down bundle (lockscreen OSK) without pads.py keeps the classic
# SC-vs-Switch behavior.
try:
...
# Close "X" button (top-right corner, shown while the mouse moves over the OSK).
# Design-time pixels at the 1286x369 layout; scaled by _font_scale per size.
_CLOSE_X_SIZE = 18
_CLOSE_X_MARGIN = 10

# Corner grab zones: a square this many design-time px on a side at each of the
# four window corners, from which a LEFT-drag slides the whole keyboard (the
# same free drag the Move key and the middle button start  see adusk
# ._begin_drag). Also design-time px, scaled per OSK size like the close "X".
_CORNER_GRAB_SIZE = 24

_active_osk_size = "medium"

# Gap between the bottom of the OSK window and the bottom of the display's
# USABLE bounds (i.e. the taskbar) at every bottom-anchored window position
# (initial placement, display-change reposition, and the down-mid/down-left/
# down-right slots of the 6-position cycle). Top-anchored positions are
# unaffected  there's no taskbar to clear up there.
TASKBAR_GAP = 10
...
# "Scale With Resolution" (Options -> Keyboard). The base size is
# resolution-independent, so on a 4K display the keyboard shrinks to a sliver
# of the screen. With this on, every size is scaled against the 1080p reference
# the layout was designed at, so the board keeps the same RELATIVE footprint
# (and font proportions  _font_scale derives from the window height) on any
# panel: 1.5x on 4K, unchanged on 1080p. Capped below the full proportional 2x
# because 1.5x already reads better across a room on a TV.
_REFERENCE_HEIGHT = 1080
_OSK_SCALE_MIN = 0.75
_OSK_SCALE_MAX = 1.5
_display_scaling = False


def set_display_scaling(enabled):
    """Turn resolution scaling on/off. Applied at the next Screen build or
    resize()  those are the only places the window size is recomputed."""
    global _display_scaling
    _display_scaling = bool(enabled)


def get_display_scaling():
    return _display_scaling


def _display_scale():
    """Size multiplier against the 1080p reference, or 1.0 when the feature is
    off or the primary display can't be queried. Reads the RAW display height,
    not the usable bounds, so a taskbar doesn't shrink the keyboard."""
    if not _display_scaling:
        return 1.0
    bounds = S.SDL_Rect()
    disp = S.SDL_GetPrimaryDisplay()
    if (disp and S.SDL_GetDisplayBounds(disp, ctypes.byref(bounds))
            and bounds.h > 0):
        return max(_OSK_SCALE_MIN,
                   min(_OSK_SCALE_MAX, bounds.h / _REFERENCE_HEIGHT))
    return 1.0


def _usable_width():
    """Primary display's usable width, or None when it can't be queried."""
    bounds = S.SDL_Rect()
    disp = S.SDL_GetPrimaryDisplay()
...
def plain_width(name=None):
    """Window width the board would have UNSPLIT at size `name` (default: the
    active size). vkb sizes each split half against this, so splitting keeps
    the keys at the size the user picked and turns a wide display's extra room
    into transparent band instead of stretching every key."""
    scale = _display_scale()
    name = _active_osk_size if name is None else name
    if name == "small":
        return int(round(_BASE_WIDTH * _SMALL_SCALE * scale))
    if name == "full":
        return _usable_width() or int(round(_BASE_WIDTH * scale))
    return int(round(_BASE_WIDTH * scale))


def _compute_size(name):
    scale = _display_scale()
    small = name == "small"
    height_px = int(round(_BASE_HEIGHT * (_SMALL_SCALE if small else 1.0)
                          * scale))
    # Split layout spans the display edge to edge whatever the size setting:
    # the two halves sit at the screen corners and the band between them is
    # rendered transparent. Only the WIDTH changes  the height still follows
    # the Size slider.
    if state.is_split_layout_enabled():
        return (_usable_width() or int(round(_BASE_WIDTH * scale)), height_px)
    if small:
        return int(round(_BASE_WIDTH * _SMALL_SCALE * scale)), height_px
    if name == "full":
        return (_usable_width() or _BASE_WIDTH), height_px
    return int(round(_BASE_WIDTH * scale)), height_px
...
class CoordFraction:
    """A point on the OSK canvas held as a 0..1 fraction of its size.

    Pointer positions are stored resolution-independently so a canvas resize
    keeps every cursor where it visually was, instead of stranding pixel
    coordinates that were measured against the old dimensions. Conversion runs
    against the module-level `width`/`height`, which the render loop updates on
    resize, so the fractions are read back through the new size automatically.
    """
    ...
    def from_absolute(x, y):
        """Build one from pixel coordinates on the current canvas."""
        return CoordFraction(x / width, y / height)

    def to_absolute(self):
        """(x, y) in pixels on the current canvas."""
        return self.x_fraction * width, self.y_fraction * height

    def update_absolute(self, x, y):
        """Move to pixel coordinates on the current canvas, in place."""
...
class Screen:
    ...
    def __init__(self):
        ...
        self._size_name = _active_osk_size  # tracked so resize() rebuilds fonts only on change
        self._size_px_h = height
        self._font_scale = height / _BASE_HEIGHT

        # Per-key press "pop": a key entering CLICK scales quickly down to
        # _PRESS_SCALE, then springs back to 1.0 when it releases  the phone
        # press feel. {(row, col): [phase, start_time]}, phase 1 = sinking,
        # 0 = springing back. Entries are dropped once settled, so an idle
        # keyboard keeps the dict empty and never holds the render loop awake.
        self._press_anim = {}
        # 0.93 opened a ~3px ring of background around a pressed key that read
        # as a glitch; 0.97 is a sub-perceptual ~1px sink.
        self._PRESS_SCALE = 0.97
        self._PRESS_IN_DUR = 0.08
        # Release spring (see utils.spring_p): enough bounce to feel springy,
        # not enough to look loose.
        self._PRESS_ZETA = 0.6
        self._PRESS_OMEGA0 = 28.0
        # When the spring is close enough to 1.0 to stop drawing it. The
        # envelope is e^(-zeta*omega0*t), so by 0.35s it is under 0.3%.
        self._PRESS_SPRING_MAX = 0.35
        ...
        # usable area (i.e. above the taskbar), leaving TASKBAR_GAP of daylight
        # between the keyboard and the taskbar instead of sitting flush on it.
        bounds = S.SDL_Rect()
        disp = S.SDL_GetPrimaryDisplay()
        if disp and S.SDL_GetDisplayUsableBounds(disp, ctypes.byref(bounds)):
            win_x = bounds.x + max(0, (bounds.w - width) // 2)
            win_y = bounds.y + max(0, bounds.h - height - TASKBAR_GAP)
        else:
        ...
        self._font_cache = {}
        # Cache of rendered per-key LABEL textures (see _get_label_texture),
        # keyed by (font identity, text, rgb). Owned by the cache  callers never
        # destroy these. Cleared on resize() and skin change (where fonts/colors
        # change). Saves re-rasterizing the static labels on every drawn frame.
        self._label_cache = {}
        ...
        # Whether to draw the top-right close "X" this frame (adusk.main sets it
        # while the mouse is moving over the OSK; clicking the X closes it).
        self.show_close_x = False
    ...
    def maybe_reload_skin(self):
        ...
        if gen != self._skin_generation:
            ...
            self._icon_alpha = min(255, int(round(204 * self._tscale))) if self._transparent else 255
            # The palette changed, so the cached labels (baked at the old colors)
            # are stale  drop them so they re-rasterize in the new skin colors.
            self._clear_label_cache()

    def resize(self, name):
        """Resize the OSK to size `name` ("small"/"medium"/"full") IN PLACE,
        reusing the existing (already-shown, transparency-capable) window  far
        more robust than building a second window mid-session, which could come
        up invisible. Rebuilds the size-dependent fonts and glyph cache and
        updates the module-level width/height the key layout reads. The caller
        repositions the window afterwards (its top-left anchor shifts on resize)
        and reflows virtual_kb.update_dimensions()."""
        global width, height
        new_w, new_h = _compute_size(name)
        # ALWAYS re-assert the globals + window size (cheap, idempotent) so a
        # drifted global is corrected even when the size name is unchanged.
        width, height = new_w, new_h
        S.SDL_SetWindowSize(self.window, new_w, new_h)
        # Keyed on the PIXEL height, not just the size name: toggling Split
        # Keyboard or Scale With Resolution moves the window under an
        # unchanged name, and the fonts have to follow it.
        if name == self._size_name and new_h == self._size_px_h:
            return  # already built at this size  fonts/glyph cache are current
        self._size_name = name
        self._size_px_h = new_h
        self._font_scale = new_h / _BASE_HEIGHT
        # Rebuild the per-size fonts (close the old ones first).
        for fm in (self.font_manager, self.font_manager_small,
                   self.font_manager_symbol, self.font_manager_symbol_small,
                   self.font_manager_shadow, self.font_manager_shadow_symbol):
            try:
                S.TTF_CloseFont(fm.font)
            except Exception:
                pass
        fp = self._font_paths["default"]
        sp = self._font_paths["symbol"]
        self.font_manager = _Font(fp, self._scaled(26))
        self.font_manager_small = _Font(fp, self._scaled(20))
        self.font_manager_symbol = _Font(sp, self._scaled(26))
        self.font_manager_symbol_small = _Font(sp, self._scaled(20))
        self.font_manager_shadow = _Font(fp, self._scaled(21))
        self.font_manager_shadow_symbol = _Font(sp, self._scaled(21))
        # Drop cached one-off fonts (rebuilt lazily at the new scale).
        for f in self._font_cache.values():
            try:
                S.TTF_CloseFont(f.font)
            except Exception:
                pass
        self._font_cache = {}
        # Labels were rasterized with the old fonts/scale  drop them too.
        self._clear_label_cache()
        # Drop cached glyph textures so they re-render at the new resolution.
        self._glyph_cache_px = max(32, self._scaled(self._GLYPH_CACHE_PX))
        for tex, _sz in self._glyph_textures.values():
            if tex:
                try:
                    ...
                    pass
        self._glyph_textures = {}
        # Drop the open-animation target (wrong size now); rebuilt on demand.
        if self._anim_target is not None:
            try:
                S.SDL_DestroyTexture(self._anim_target)
            except Exception:
                pass
            self._anim_target = None
    ...
    def _font_for_char(self, ch):
        """The label font if it has a glyph for `ch`, else the symbol font.

        Only the accent row asks, and only for the handful of candidates on
        screen, so the per-character probe is not on a hot path. Any failure
        of the SDL_ttf query falls back to the label font  a tofu box beats
        raising mid-render."""
        if not ch:
            return self.font_manager
        try:
            if S.TTF_FontHasGlyph(self.font_manager.font, ord(ch[0])):
                return self.font_manager
            if S.TTF_FontHasGlyph(self.font_manager_symbol.font, ord(ch[0])):
                return self.font_manager_symbol
        except Exception:
            pass
        return self.font_manager

    def press_anim_active(self):
        """True while any key is mid press-pop. The animation is time-based,
        so the main loop's dirty-flag signature cannot see it  the loop asks
        this instead and keeps drawing until it goes quiet."""
        return bool(self._press_anim)

    def _press_scale(self, key_id):
        """Current press scale for a key (1.0 when idle). `key_id` is a
        (row, col), or anything carrying .row/.col. Pure: render_vkb advances
        the phases, this only reads them, so it stays cheap per key."""
        ...
        now = time.monotonic()
        if phase == 1:
            # Sinking: fast ease-in that settles on the pressed scale.
    ...
    def _render_split_background(self, virtual_kb):
        """Split layout, opaque mode: erase the whole window to alpha 0 and
        repaint ONLY the two keyboard halves, so the middle band stays fully
        transparent and the desktop shows through it instead of a black slab.
        Transparent mode already erases everywhere, so this never runs there."""
        ...
        S.SDL_RenderClear(self.renderer)
        S.SDL_SetRenderDrawColor(self.renderer, c.r, c.g, c.b, 255)
        S.SDL_RenderFillRect(self.renderer, ctypes.byref(
            S.SDL_FRect(0.0, 0.0, float(left_x), float(height))))
        S.SDL_RenderFillRect(self.renderer, ctypes.byref(
            S.SDL_FRect(float(right_x), 0.0, float(width - right_x),
                        float(height))))

    def _render_diacritic_row(self):
        """Overlay pass for the accent row: a strip of candidate keys floating
        just above the held letter. The highlighted candidate is painted in the
        pressed colors; a base selection (index -1) highlights none.

        Drawn after the keys, and drawn as a real popup  opaque panel, hard
        1px rim, small drop shadow  because it sits ON TOP of the key row
        above the held key. Without the rim and shadow it would read as those
        keys lighting up rather than as a floating strip."""
        ...
        # Shadow first, offset down+right; the panel covers all but the sliver.
        S.SDL_SetRenderDrawColor(self.renderer, rim.r, rim.g, rim.b, 110)
        S.SDL_RenderFillRect(self.renderer,
                             ctypes.byref(S.SDL_FRect(x + 2, y + 2, w, h)))
        S.SDL_SetRenderDrawColor(self.renderer, panel.r, panel.g, panel.b, 255)
        S.SDL_RenderFillRect(self.renderer,
                             ctypes.byref(S.SDL_FRect(x, y, w, h)))
        ...
        # Candidates laid out on the SHARED geometry constants the input paths
        # map pointer x through, so the box drawn and the box selected can
        # never disagree.
        # Recover the candidate size from the strip itself, so the boxes
        # drawn and the boxes the pointer selects can never disagree at a
        # size the strip was shrunk for.
        step = diacritics.candidate_step(rect, len(variants))
        cw = step - diacritics.CANDIDATE_GAP
        pad_y = 4 if h > 4 * 4 else 1
        cand_h = h - 2 * pad_y
        for i, ch in enumerate(variants):
            cx = x + int(round(i * step))
            cy = y + pad_y
            pressed = i == index
            fill = self.key_color[state.InputState.CLICK if pressed
                                  else state.InputState.INACTIVE]
            label_color = self.text_color[state.InputState.CLICK if pressed
                                          else state.InputState.INACTIVE]
            S.SDL_SetRenderDrawColor(self.renderer, fill.r, fill.g, fill.b, 255)
            S.SDL_RenderFillRect(
                self.renderer, ctypes.byref(S.SDL_FRect(cx, cy, cw, cand_h)))
            # The label font (Selawik) has no glyph for a few of the rarer
            # accents  the Esperanto circumflex/breve set among them  and
            # would draw them as tofu boxes. The bundled symbol font (DejaVu)
            # covers every character the variant map can produce, so fall back
            # to it per character.
            tex, tw, th = self._make_text_texture(self._font_for_char(ch), ch,
                                                  label_color)
            if not tex:
                continue
            S.SDL_SetTextureAlphaMod(tex, self._text_alpha)
            dst = S.SDL_FRect(cx + (cw - tw) // 2, cy + (cand_h - th) // 2,
                              tw, th)
    ...
    def _get_label_texture(self, font_obj, text, color):
        """Cached variant of _make_text_texture for the static per-key LABELS,
        which would otherwise be re-rasterized (surface -> texture) on EVERY
        frame. Keyed by (font identity, text, rgb); the texture is OWNED by the
        cache, so callers must NOT destroy it. Cleared on resize() / skin change
        (where fonts or colors change). Returns (texture, w, h) or (None, 0, 0)."""
        key = (id(font_obj), text, color.r, color.g, color.b)
        entry = self._label_cache.get(key)
        if entry is None:
            entry = self._make_text_texture(font_obj, text, color)
            self._label_cache[key] = entry
        return entry

    def _clear_label_cache(self):
        """Destroy and drop every cached label texture. Called when the fonts or
        palette change (resize / skin), which invalidates the rasterized labels."""
        for tex, _w, _h in self._label_cache.values():
            if tex:
                try:
                    S.SDL_DestroyTexture(tex)
                except Exception:
    ...
    # Phone-layout corner hint: font size in base-layout px, and the inset from
    # the key's top-right corner.
    _HINT_FONT_SIZE = 12
    _HINT_INSET = 5

    def _draw_hint(self, key, hint, font="default"):
        """Draw the phone layout's long-press hint in a key's top-right corner
        (Q's '%', A's '@', ...). Cached like any other static label  it never
        changes with shift or key state."""
        font_obj = self._get_sized_font(font, self._HINT_FONT_SIZE)
        tex, tw, th = self._get_label_texture(
            font_obj, hint, self.shadow_label_color)
        if not tex:
            return
        inset = self._scaled(self._HINT_INSET)
        dst = S.SDL_FRect(key.x + key.w - tw - inset, key.y + inset, tw, th)
        S.SDL_RenderTexture(self.renderer, tex, None, ctypes.byref(dst))

    def render_key(self, txt, key, key_state, modifier=False, align="center", valign="center", glyph=None, font="default", text_color_override=None, bg_color_override=None, shadow_label=None, font_size=None, text_dx=0, shadow_font_size=None, dual_anim=None, outline_px=None, outline_opacity=None, glyph_valign=None, hint=None):
        ...
        if glyph:
            self._draw_glyph(key, glyph, align, txt, glyph_color, valign=glyph_valign)
        ...
        # Cached: the per-key label is re-blitted (not re-rasterized) each frame.
        tex, tw, th = self._get_label_texture(font_obj, txt, label_color)
        if not tex:
            return
    ...
    def _draw_glyph(self, key, glyph, align, txt, glyph_color, dy=0.0, alpha=255, valign=None):
        ...
        otherwise it sits at the bottom on the opposite side from the label,
        UNLESS `valign` is "center" (e.g. Backspace's X icon beside its text on
        one line), in which case it's vertically centered instead. `dy` shifts it
        vertically and `alpha` scales its opacity  used by the Shift animation
        to slide the Move key's keyboard icon down and fade it out."""
        ...
        if txt == "":
            ...
            gx = key.x + (key.w - gw_draw) // 2
        if is_centerpiece or valign == "center":
    ...
    # Swipe trail: how many of the most recent points are drawn, the line
    # thickness in base-layout px, and how many equal chunks the tail is split
    # into (each chunk is one SDL_RenderLines call at its own alpha, which is
    # what fades the tail out behind the finger without a call per segment).
    _TRAIL_POINTS = 150
    _TRAIL_WIDTH = 4
    _TRAIL_CHUNKS = 5

    def _render_swipe_trails(self):
        """Draw the live Swipe Typing trails behind the pointer. Without this
        the gesture is invisible and effectively unusable  the trail is what
        tells the user which letters the path actually took in."""
        trails = state.get_swipe_trails()
        if not trails:
            return
        r = self.renderer
        col = self.key_color[state.InputState.CLICK]
        S.SDL_SetRenderDrawBlendMode(r, S.SDL_BLENDMODE_BLEND)
        half = max(0.5, self._scaled(self._TRAIL_WIDTH) / 2.0)
        for pts in trails:
            if len(pts) < 2:
                continue
            if len(pts) > self._TRAIL_POINTS:
                pts = pts[-self._TRAIL_POINTS:]
            n = len(pts)
            step = max(2, n // self._TRAIL_CHUNKS)
            lo = 0
            while lo < n - 1:
                hi = min(n, lo + step + 1)     # +1 so chunks share an endpoint
                seg = pts[lo:hi]
                # Newer chunks are brighter, so the trail reads as a comet.
                alpha = int(35 + 215 * (hi - 1) / (n - 1))
                S.SDL_SetRenderDrawColor(r, col.r, col.g, col.b, alpha)
                count = len(seg)
                buf = (S.SDL_FPoint * count)()
                # Three parallel passes fake a thick line: SDL_RenderLines is
                # always 1px, and a 1px trail is far too thin to follow.
                for off in (-half, 0.0, half):
                    for i, (px, py) in enumerate(seg):
                        buf[i].x = px + off
                        buf[i].y = py + off
                    S.SDL_RenderLines(r, buf, count)
                lo = hi - 1

    def render_vkb(self, virtual_kb, pointers):
        # Keep the swipe decoder's letter-key centres in step with the live
        # layout. Cheap no-op unless the layout actually changed, and doing it
        # here means every render path (main loop, preview, open animation)
        # publishes it without each having to remember to.
        swipe.set_key_geometry(virtual_kb)
        # Same reasoning: publishing the board here means every render path
        # (main loop, preview, open animation) hands the input thread the
        # CURRENT page without each having to remember to.
        ...
        select_active = state.is_select_active()
        # Latched on-screen modifiers, read from the latches rather than from
        # `highlighted` (which also carries physically-held buttons  those
        # ARE presses and should still pop).
        latched_mods = state.get_latched_modifier_keys()
        # Swap the Shift/Enter trigger hint glyphs to match the controller last
        # used on the OSK: each SDL pad family shows its own trigger art
        # (Switch ZL/ZR, Xbox LT/RT, PS L2/R2, Deck L2/R2, ...) and the Steam
        # Controller its custom L2/R2 hints. Read once per frame.
        _active_kind = state.get_active_controller()
        if _pads is not None:
            hint_glyphs = _pads.osk_hint_glyphs(_active_kind)
            xy_swap = _pads.nintendo_xy(_active_kind)
        else:
            hint_glyphs = (("switchpro_l2_md.png", "switchpro_r2_md.png")
                           if _active_kind != "sc" else None)
            xy_swap = _active_kind != "sc"
        ...
        for key in virtual_kb.gen_key_layouts():
            ...
            # Spacers only reserve width (the phone layout's half-key indent on
            # the A-L row). Drawing nothing at all  not even a background 
            # keeps them from lighting up under the pointer.
            if kb_key.spacer:
                continue
            ...
            if (mouse_press_cell is not None
                ...
                input_state = max(state.InputState.CLICK, input_state)
            # Select key: held down for the whole drag-to-select gesture, and
            # the finger leaves it almost immediately  so the highlight comes
            # from the session flag, not from where the pointer is.
            if select_active and kb_key.is_select:
                input_state = max(state.InputState.CLICK, input_state)

            # Press pop: sink on the CLICK rising edge, spring back on the
            # falling one. A LATCHED modifier (Shift/Ctrl/Alt toggled ON on
            # screen) is a held STATE, not a press  it keeps the CLICK colour
            # but must never sink, or it would sit sunken for as long as the
            # latch is on. Physically-held buttons are NOT excluded: those
            # really are presses and should sink for as long as they're down.
            pressed_now = input_state == state.InputState.CLICK
            key_id = (key.row, key.col)
            _entry = self._press_anim.get(key_id)
            if kb_key.keycode and kb_key.keycode in latched_mods:
                self._press_anim.pop(key_id, None)
            elif pressed_now and _entry is None:
                self._press_anim[key_id] = [1, now_t]
            elif not pressed_now and _entry is not None:
                if _entry[0] == 1:
                    self._press_anim[key_id] = [0, now_t]
                elif now_t - _entry[1] >= self._PRESS_SPRING_MAX:
                    # Settled  drop it so an idle board stops re-rendering.
                    self._press_anim.pop(key_id, None)
            ...
            if glyph == "sc_r2_md.png" and rpad_touched:
                glyph = None
            # An SDL pad last used → show its family's glyphs in place of the
            # Steam Controller's. Done after the touch-hide check so every
            # controller's hints hide the same way; the YAML only ever carries
            # the SC glyph names.
            #   * L2/R2 (Shift/Enter) → that family's trigger art (Switch
            #     ZL/ZR, Xbox LT/RT, PS L2/R2, ...  pads.osk_hint_glyphs).
            #   * X (Backspace) / Y (Space) glyphs are SWAPPED on Nintendo
            #     layouts only: Nintendo places the physical X and Y buttons
            #     opposite the Xbox/Steam layout, so the SC's "X" button sits
            #     where the Switch's "Y" is and vice versa  swapping the
            #     icons keeps each hint on the button the user actually presses.
            if hint_glyphs is not None:
                if glyph == "glyph_l2.png":
                    glyph = hint_glyphs[0]
                elif glyph == "sc_r2_md.png":
                    glyph = hint_glyphs[1]
            if xy_swap:
                if glyph == "glyph_x.png":
                    glyph = "glyph_y.png"
            ...
            # Press pop: scale the whole key  fill, label and glyph  about
            # its centre. The clip above stays at the ORIGINAL bounds, so a
            # sunken key shows the background around it (the sink is what you
            # see) and a spring overshoot past 1.0 can't bleed into a
            # neighbour.
            _pscale = self._press_scale(key)
            rkey = key
            if _pscale != 1.0:
                _nw = max(1, int(round(key.w * _pscale)))
                _nh = max(1, int(round(key.h * _pscale)))
                rkey = key._replace(x=key.x + (key.w - _nw) // 2,
                                    y=key.y + (key.h - _nh) // 2,
                                    w=_nw, h=_nh)
            self.render_key(label, rkey, input_state, modifier=kb_key.modifier,
                            ...
                            outline_px=kb_key.outline_px,
                            outline_opacity=kb_key.outline_opacity,
                            glyph_valign=kb_key.glyph_valign,
                            hint=kb_key.hint)

        # Retire press-pop entries for cells the CURRENT board doesn't have.
        # Switching layout or page (the phone board's ?123 pages) swaps the
        # grid under a key that is still animating; its entry would then never
        # be visited by the loop above, so it could never settle  and
        # press_anim_active() would hold the render loop awake forever. Costs
        # nothing while the dict is empty, which is the normal case.
        if self._press_anim:
            for _cell in list(self._press_anim):
                _r, _c = _cell
                if _r >= len(virtual_kb.keys) or _c >= len(virtual_kb.keys[_r]):
                    self._press_anim.pop(_cell, None)
    ...
    def _close_x_geom(self):
        """Rect (x, y, w, h) of the top-right close "X", scaled to the current
        OSK size and read off the live module width (so it tracks resizes)."""
        sz = self._scaled(_CLOSE_X_SIZE)
        margin = self._scaled(_CLOSE_X_MARGIN)
        return width - sz - margin, margin, sz, sz

    def close_x_hit(self, mx, my):
        """True if window-relative point (mx, my) is on the close "X" button."""
        x, y, w, h = self._close_x_geom()
        return x <= mx <= x + w and y <= my <= y + h

    def corner_grab_hit(self, mx, my):
        """True if window-relative point (mx, my) is in one of the four corner
        grab zones  the outermost corners of the keyboard, where a left-drag
        moves the window instead of typing. Costs no key area: a press that
        never travels still types the key under it (adusk's tap fallback). The
        close "X" is tested first by the caller, so in the top-right corner this
        only claims the strip of margin around it."""
        sz = self._scaled(_CORNER_GRAB_SIZE)
        return ((mx <= sz or mx >= width - sz)
                and (my <= sz or my >= height - sz))

    def _render_close_x(self, selected=False):
        """Draw an "X" cross (no background box) tinted with the active skin's
        solid (idle) key color, so it matches every skin. When *selected*
        (controller cursor on it), it brightens toward white so it still reads
        as highlighted."""
        x, y, w, h = self._close_x_geom()
        r = self.renderer
        base = self.key_color[state.InputState.INACTIVE]
        if selected:
            red, green, blue = (min(255, base.r + 55), min(255, base.g + 55), min(255, base.b + 55))
        else:
            red, green, blue = base.r, base.g, base.b
        S.SDL_SetRenderDrawColor(r, red, green, blue, 255)
        pad = w * 0.25
        x0, y0 = x + pad, y + pad
        x1, y1 = x + w - pad, y + h - pad
        for d in (-1.5, -0.5, 0.5, 1.5):
            S.SDL_RenderLine(r, x0 + d, y0, x1 + d, y1)
            S.SDL_RenderLine(r, x0 + d, y1, x1 + d, y0)
    ...
    def render_close_anim(self, virtual_kb, pointers, fade, scale=1.0):
        ...
        open: the board fades out (fade 1->0) while scaling down slightly
        about its centre, so it recedes rather than blinking away.

        Same offscreen-target composite as render_open_anim: the keyboard is
        drawn at FULL opacity into the target (so the font alpha is never
        scaled), then blitted faded. Returns False when no offscreen target is
        available, and the caller falls back to an instant hide."""
        ...
        S.SDL_RenderClear(self.renderer)
        if fade > 0.0:
            alpha = 255 if fade >= 1.0 else int(round(fade * 255))
            S.SDL_SetTextureAlphaMod(tex, alpha)
            # The target holds PREMULTIPLIED RGB (see _ensure_anim_target), so
            # the colour has to be scaled by the same alpha  without it the
            # closing keyboard washes the desktop white on its way out.
            S.SDL_SetTextureColorMod(tex, alpha, alpha, alpha)
            sw = max(1, int(round(width * scale)))
            sh = max(1, int(round(height * scale)))
            dst = S.SDL_FRect((width - sw) / 2.0, (height - sh) / 2.0,
                              float(sw), float(sh))
            S.SDL_RenderTexture(self.renderer, tex, None, ctypes.byref(dst))
    ...
    def render(self, virtual_kb, pointers):
        self.clear()
        if not self._transparent:
            # Split layout: repaint just the two halves, leaving the middle
            # band transparent (opaque mode's clear() filled the whole window).
            self._render_split_background(virtual_kb)
        self.render_vkb(virtual_kb, pointers)
        self._render_diacritic_row()
        # Under the pointer circles, over the keys.
        self._render_swipe_trails()
        ...
        if pointers[1].state != state.InputState.INACTIVE:
            self.render_ptr(pointers[1])
        if self.show_close_x:
            _cx_sel = False
            for _p in pointers:
                if _p.state != state.InputState.INACTIVE:
                    _px, _py = _p.coord_frac.to_absolute()
                    if self.close_x_hit(_px, _py):
                        _cx_sel = True
                        break
            self._render_close_x(selected=_cx_sel)
        S.SDL_RenderPresent(self.renderer)
