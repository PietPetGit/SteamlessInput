...
SHORTCUT_NAME = "SteamlessInput.lnk"
...
def create_shortcut(lnk_path, target, arguments="", workdir="", icon=None):
...
# Kept as a private alias: this module authored the .lnk long before the install
# wizard (installer.py) needed the same COM dance for its Desktop / Start Menu
# shortcuts. One implementation, two callers.
_create_shortcut = create_shortcut
...
def shortcut_path():
    """Absolute path of the Startup-folder shortcut, or None if the folder
    can't be resolved. Public so the install wizard can create/remove the same
    entry the tray toggles."""
...
def enable_for(target_exe):
    """Create the Startup-folder shortcut pointing at a SPECIFIC executable.

    ``enable()`` points the shortcut at the *running* process, which is right
    for the tray toggling itself but wrong for the install wizard: that is a
    different exe installing a third one. Same shortcut, same folder, explicit
    target."""
    path = _shortcut_path()
    if not path:
        return False
    ...
    return _create_shortcut(path, target, "", os.path.dirname(target), target)
