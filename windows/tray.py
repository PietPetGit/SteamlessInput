...
# pystray marks the left-click ("default") menu item bold on Win32
# (MFS_DEFAULT) even in an otherwise plain menu  Manager is the left-click
# target here but shouldn't read differently from Keyboard/Exit. Left-click
# dispatch itself is pure Python (pystray._base.Menu.__call__ scans for
# `.default` and doesn't touch this bit), so zeroing it only drops the bold.
from pystray._util import win32 as _pystray_win32  # noqa: E402
_pystray_win32.MFS_DEFAULT = 0
...
import uia_client  # noqa: E402

# Hand the injection layer its route to the uiAccess relay. Registering it
# costs nothing  the relay is only launched if an administrator window
# actually blocks us (see _UipiGuard.update).
sui.set_relay(uia_client.CLIENT)
...
import big_picture  # noqa: E402  (import-safe: ctypes/winreg only  the
#                     Options → Big Picture automation engine)
...
DEFAULT_SETTINGS = {
    ...
    "tutorial_done": False,
    # Options → General "Keep Working Over Elevated Windows": while a window
    # that outranks us in integrity owns the foreground, Windows discards every
    # event we inject (see _UipiGuard), so we hand the cursor back to the
    # controller's own firmware for the duration. On by default  the
    # alternative is a dead trackpad over Task Manager and the UAC prompt.
    "elevated_fallback": True,
    ...
    # disappears we hold its virtual XInput device open (zeroed) for
    ...
    # Pro-Controller-shaped pad. Off, each half is its own controller  which
    # with automatic multiplayer means two players, one Joy-Con each. Read by
    # SDL at init, so it applies on the next launch.
    "joycon_combine": True,
    ...
    # game is running  Steam's RunningAppID guard).
    "bp_auto_close": False,
    # While Big Picture is open (each reverts when it closes):
    "bp_display": False,          # switch to a single chosen display
    "bp_display_device": "",      # its DisplayConfig monitorDevicePath
    "bp_audio": False,            # switch the default playback device
    "bp_audio_device": "",        # its MMDevice endpoint id
    "bp_hdr": False,              # force HDR on (capable displays)
    "bp_nightlight": False,       # disable Night Light (semantic restore)
    "bp_pause_media": False,      # pause everything playing (SMTC)
    "bp_hide_cursor": False,      # blank the mouse cursor (move reveals)
    # Crash-recovery record: the live session's revert data (display topology,
    # previous audio device, night light snapshot, ...), persisted the moment
    # a session starts and cleared on its normal exit  a crash mid-session is
    # repaired on the next launch. A dict, NOT a bool.
    "bp_recovery": {},
...
# --- Elevated-window (UIPI) input guard -------------------------------------
# Windows' User Interface Privilege Isolation silently discards every event a
# medium-integrity process injects while a HIGHER-integrity window owns the
# foreground: Task Manager, regedit, an elevated console, an installer, the UAC
# consent desktop. Steam Input has exactly this hole -- open Task Manager and
# the trackpad cursor freezes -- and so did we, because the desktop takeover
# opens the controller with passive=False (firmware lizard mode OFF) and then
# drives everything through SendInput.
#
# The fix is to hand control back to the hardware. Lizard mode's mouse and
# keyboard reports come off the controller as ordinary HID input, entering
# through the driver stack rather than the injection path, so UIPI never
# applies to them -- they work over Task Manager AND over the UAC prompt, which
# no user-mode privilege can reach. The trade is that lizard mode is the
# firmware's own fixed mapping, not the user's binds, so the guard announces
# itself (haptic tick + a one-shot toast) instead of looking like another
# glitch.
#
# Detection is two-part:
#   * the input desktop isn't ours -> the secure desktop (UAC consent) or the
#     lock screen is up, and nothing we inject lands anywhere;
#   * the foreground window's process sits above us in integrity -- or refuses
#     even a query-limited handle, which means the same thing in practice.

_SECURITY_MANDATORY_MEDIUM_RID = 0x2000
_TOKEN_INTEGRITY_LEVEL = 25
_TOKEN_QUERY = 0x0008
_PROCESS_QUERY_LIMITED_INFORMATION = 0x1000
_UIPI_RECHECK_SEC = 0.5
...
def _token_integrity_rid(proc_handle):
    """Integrity-level RID of a process handle (0x2000 = medium, 0x3000 = high,
    0x4000 = system), or None if the token can't be read."""
    advapi = ctypes.windll.advapi32
    # argtypes are load-bearing: GetCurrentProcess()'s pseudo-handle comes back
    # as 0xFFFFFFFFFFFFFFFF once its restype is HANDLE, and ctypes defaults an
    # un-annotated argument to c_int  which raises OverflowError instead of
    # passing the handle. That exception used to be swallowed by the caller's
    # try/except and silently reported us as MEDIUM integrity, so an app
    # running elevated would think every ordinary window outranked it.
    advapi.OpenProcessToken.argtypes = [wintypes.HANDLE, wintypes.DWORD,
                                        ctypes.POINTER(wintypes.HANDLE)]
    tok = wintypes.HANDLE()
    if not advapi.OpenProcessToken(proc_handle, _TOKEN_QUERY, ctypes.byref(tok)):
        return None
    try:
        size = wintypes.DWORD(0)
        # First call always "fails" (ERROR_INSUFFICIENT_BUFFER)  it's how the
        # label's size (SID length is variable) comes back.
        advapi.GetTokenInformation(tok, _TOKEN_INTEGRITY_LEVEL, None, 0,
                                   ctypes.byref(size))
        if not size.value:
            return None
        buf = ctypes.create_string_buffer(size.value)
        if not advapi.GetTokenInformation(tok, _TOKEN_INTEGRITY_LEVEL, buf,
        ...
        count = advapi.GetSidSubAuthorityCount(label.Label.Sid).contents.value
        if not count:
            return None
        return int(advapi.GetSidSubAuthority(
...
def _own_integrity_rid():
    """Our own integrity RID (cached  it can't change while we run)."""
    global _own_rid_cache
    if _own_rid_cache is None:
        try:
            k32 = ctypes.windll.kernel32
            # restype matters: the pseudo-handle is -1 as a HANDLE, and letting
            # ctypes default it to c_int is the usual way to lose the top half
            # of a handle on x64.
            k32.GetCurrentProcess.restype = wintypes.HANDLE
            rid = _token_integrity_rid(k32.GetCurrentProcess())
        except Exception:
            rid = None
        _own_rid_cache = rid if rid is not None else _SECURITY_MANDATORY_MEDIUM_RID
    return _own_rid_cache


def _process_integrity_rid(pid):
    """Integrity RID of `pid`, or None when we can't even open it for a
    limited query  which is itself the answer we care about (a process that
    refuses us a handle is one we can't inject into either)."""
    if not pid:
        return None
    k32 = ctypes.windll.kernel32
    ...
    if not h:
        return None
    try:
        return _token_integrity_rid(h)
    finally:
        k32.CloseHandle(h)


def _input_desktop_is_ours():
    """False while the INPUT desktop belongs to someone else  the UAC consent
    desktop or the lock screen (Winlogon). Nothing we inject reaches those, so
    the guard treats it exactly like an elevated foreground window."""
    u = ctypes.windll.user32
...
def _hwnd_injectable(hwnd):
    """True if we could inject into `hwnd`'s process  i.e. it does NOT sit
    above us in integrity. Window-local: the input-DESKTOP check belongs to
    the caller (it's a global condition, and re-running it per window during
    an EnumWindows sweep would be pure waste)."""
    pid = _hwnd_pid(hwnd)
    if not pid:
        return False
    if pid == os.getpid():
        return True
    rid = _process_integrity_rid(pid)
    if rid is None:
        return False            # couldn't query it => can't inject into it
    return rid <= _own_integrity_rid()


def _foreground_blocks_injection(hwnd):
    """True when injected input aimed at `hwnd` would be dropped by UIPI."""
    try:
        if not _input_desktop_is_ours():
            return True
        if not hwnd:
            # No foreground window at all  normally a transient desktop
            # state, not a blocked one. The genuinely blocked case (secure
            # desktop) is already caught above.
            return False
        return not _hwnd_injectable(hwnd)
    except Exception:
        return False            # never wedge the input path on a probe error


def _hwnd_is_rescuable(hwnd):
    """True if `hwnd` is still a real, visible, non-elevated window we could
    hand the foreground back to. Our OWN windows don't count  handing the
    foreground to the tray/picker is not an escape from anything."""
    if not hwnd:
        return False
    try:
        u = ctypes.windll.user32
        hp = ctypes.c_void_p(hwnd)
        if not u.IsWindow(hp) or not u.IsWindowVisible(hp):
            return False
        if _hwnd_pid(hwnd) == os.getpid():
            return False
        return _hwnd_injectable(hwnd)
    except Exception:
        return False


def _first_rescuable_window():
    """Topmost visible, titled, non-elevated top-level window  the fallback
    target when we have no remembered one (e.g. the guard engaged before we
    ever saw a normal foreground). EnumWindows walks in z-order, so the first
    hit is the one the user would have alt-tabbed to anyway."""
    found = []

    # c_void_p (not wintypes.HWND) for the handle, and every user32 call below
    # gets an explicit c_void_p: a bare Python int argument is marshalled as a
    # 32-bit c_int, which TRUNCATES real x64 handles and makes every probe in
    # here quietly answer "no".
    @ctypes.WINFUNCTYPE(wintypes.BOOL, ctypes.c_void_p, ctypes.c_void_p)
    def _cb(hwnd, _lparam):
        u = ctypes.windll.user32
        hp = ctypes.c_void_p(hwnd)
        if not u.IsWindowVisible(hp):
            return True
        if u.GetWindowTextLengthW(hp) <= 0:
            return True
        # WS_EX_TOOLWINDOW  skip palettes/tray helpers, not somewhere a user
        # would want the foreground handed to.
        if u.GetWindowLongW(hp, -20) & 0x00000080:
            return True
        if not _hwnd_is_rescuable(hwnd):
            return True
...
def _force_foreground(hwnd):
    """Hand the OS foreground to `hwnd`. Deliberately touches only the TARGET
    window and our own process  never the elevated window we're escaping 
    so UIPI has nothing to block. Zeroing the foreground lock timeout is what
    makes SetForegroundWindow succeed from a process that doesn't already own
    the foreground; SwitchToThisWindow is the fallback when it still refuses.
    Returns True if the foreground actually moved."""
    ...
    changed = False
    try:
        u.SystemParametersInfoW(SPI_GETFOREGROUNDLOCKTIMEOUT, 0,
                                ctypes.byref(old), 0)
        changed = bool(u.SystemParametersInfoW(
            SPI_SETFOREGROUNDLOCKTIMEOUT, 0, ctypes.c_void_p(0), 0x0002))
    except Exception:
        pass
    try:
        if u.IsIconic(hp):
            u.ShowWindow(hp, 9)                      # SW_RESTORE
        try:
        ...
        if changed:
            try:
                u.SystemParametersInfoW(SPI_SETFOREGROUNDLOCKTIMEOUT, 0,
                                        ctypes.c_void_p(old.value), 0x0002)
            except Exception:
                pass
...
def _uac_enabled():
    """Whether User Account Control is on. uiAccess is part of the UAC
    machinery, so with EnableLUA=0 Windows grants it to nobody  and equally
    nothing is blocked in the first place, because every process then runs at
    high integrity. Reported in Options so "helper installed but inactive"
    reads as the non-problem it is."""
    try:
        import winreg
        with winreg.OpenKey(
        ...
        return True             # assume the default (on) if unreadable


def relay_status():
    """Status code for the Options → General input-helper row:
      "ok"             installed, and UAC is on so it can be granted
      "uac_off"        installed but UAC is disabled (and unnecessary)
      "not_installed"  no helper; the app falls back to lizard mode
    Deliberately does NOT launch the relay to check: opening the Options page
    should not start a privileged process."""
    if not sui.relay_installed():
        return "not_installed"
    return "ok" if _uac_enabled() else "uac_off"


def install_relay_elevated():
    """Run install_uia_relay.ps1 under the UAC prompt (one consent, once).
    The script signs the helper and copies it into %ProgramFiles%  both need
    administrator rights; the relay itself never runs elevated afterwards."""
    script = os.path.join(_exe_dir(), "install_uia_relay.ps1")
    if not os.path.isfile(script):
        print("install_uia_relay.ps1 not found next to the exe")
        return False
    try:
        rc = ctypes.windll.shell32.ShellExecuteW(
            None, "runas", "powershell.exe",
            '-NoProfile -ExecutionPolicy Bypass -File "%s"' % script,
            None, 1)
        return int(rc) > 32
    except Exception as e:
        print(f"relay install launch failed: {e!r}")
        return False


class _UipiGuard:
    """Engages the lizard-mode fallback while injected input can't land.

    Driven from the input loop (one cheap GetForegroundWindow per frame, the
    token probe only on a foreground CHANGE or every _UIPI_RECHECK_SEC), so
    engaging costs a frame rather than a poll interval and disengaging happens
    the moment the user pads back out of the elevated window. Module-global
    because the watcher is rebuilt on every mode change / OSK toggle / keybind
    save, and a half-torn-down watcher must not leave injection gated."""
    ...
    def __init__(self):
        ...
        # True while injection is being routed through the uiAccess relay
        # rather than dropped for lizard mode.
        self._relay_on = False
        self._route_checked = 0.0
        # Paused-for-Steam escape: when the last block was seen. The passive
        # reader that watches for the escape chord stays open for
        # _ESCAPE_GRACE_S past the block, so hopping between Task Manager and
        # a normal window doesn't open/close the shared HID over and over.
        self._last_blocked_at = 0.0

    _ESCAPE_GRACE_S = 30.0

    def set_enabled(self, on):
        """Options → General toggle. Turning it off while engaged releases
        immediately (the caller passes the live controller in on the next
        frame; reset() handles the no-controller case)."""
    ...
    @property
    def enabled(self):
        """The Options → General toggle. Gates the whole feature, escape
        included  a user who turned it off shouldn't find one half of it
        still firing."""
        return self._enabled

    def blocked(self, fast=False):
        """Cached "is our injection going nowhere right now".

        `fast` re-probes ONLY when the foreground window actually changed,
        skipping the periodic re-check  for the paused-for-Steam poll, where
        sub-second freshness buys nothing and the whole point is to cost
        almost nothing while idle. That path is then exactly one
        GetForegroundWindow per tick until the user switches windows."""
        now = time.monotonic()
        hwnd = _foreground_hwnd()
        if hwnd != self._hwnd or (not fast
                                  and (now - self._checked) >= _UIPI_RECHECK_SEC):
            self._hwnd = hwnd
            self._checked = now
            self._blocked = _foreground_blocks_injection(hwnd)
            if self._blocked:
                self._last_blocked_at = now
            if not self._blocked and hwnd:
                # Remember where to hand the foreground BACK to (the rescue
                # action's target), skipping our own windows  the picker being
                # focused is not a state anyone needs rescuing from.
                pid = _hwnd_pid(hwnd)
                if pid and pid != os.getpid():
                    self._last_normal = hwnd
        return self._blocked

    def update(self, sc, chord=None, kind=None):
        """Per-frame edge handling. Returns True while engaged.

        Two ways to survive an administrator window, in preference order:

          1. the uiAccess RELAY  exempt from the UIPI filter, so the user's
             real bindings, the pad cursor and the OSK all keep working; or
          2. firmware LIZARD MODE  always available on a Steam Controller /
             Deck, but a fixed layout that parks the user's bindings.

        The relay wins whenever it's genuinely privileged, which is why the
        check is relay_ready() (token-verified) and not merely "installed"."""
        want = self._enabled and self.blocked()
        if want and not self._engaged:
            # Only try to bring the relay up on the way IN, and only when we
            # actually need it  starting a privileged helper speculatively,
            # or retrying it every frame, would both be wrong.
            sui.relay_start()
        use_relay = want and sui.relay_ready()
        if use_relay != self._relay_on:
            self._relay_on = use_relay
            sui.set_relay_active(use_relay)
        if want == self._engaged:
            return self._engaged
        self._engaged = want
        if want and chord is not None:
            # Drop held modifiers BEFORE going quiet so nothing is stranded at
            # the OS level for the duration (releases are never gated  see
            # uinput._suppressed).
            try:
                chord.release_all_held()
            except Exception:
                pass
        # Lizard mode is the FALLBACK, so it only comes on when the relay
        # isn't carrying. Same for the injection gate: with the relay live our
        # own events must keep flowing (to it), not be dropped.
        want_lizard = want and not self._relay_on
        try:
            sc.set_lizard(want_lizard)
        except Exception:
            pass
        sui.set_suppressed(want_lizard)
        if want:
            try:
                if kind is None or adusk_state.is_rumble_enabled(kind):
                    sc.haptic_click()
            except Exception:
                pass
            if not self._announced and self._notifier is not None:
                self._announced = True
                try:
                    if self._relay_on:
                        # Nothing is lost in this mode, so the toast is a
                        # courtesy rather than an explanation of missing binds.
                        self._notifier(
                            "Administrator window",
                            "Windows blocks input sent by a program to this "
                            "window. Routing through the input helper, so "
                            "your bindings keep working.")
                    else:
                        self._notifier(
                            "Administrator window  lizard mode",
                            "Windows blocks input sent by a program to this "
                            "window, so the Steam Controller is back in "
                            "lizard mode (its built-in mouse). Hold Steam and "
                            "press View to leave; your bindings come back "
                            "when you do.")
                except Exception:
                    pass
        return self._engaged

    # How often an input source without an engage/disengage cycle re-checks
    # the relay route. 4 Hz is invisible against a 250 Hz pad loop and only
    # runs at all once a relay is installed.
    _ROUTE_POLL_S = 0.25

    def ensure_relay_route(self):
        """Keep the relay route in sync for input sources that never engage.

        SDL pads have no lizard mode, so they never run update()'s engage /
        disengage cycle  without this they'd stay on the dead SendInput path
        even with the helper installed, which was the whole point of building
        it. Safe to call from any input thread alongside the Steam Controller
        path: "is the foreground blocked" is a property of the WINDOW, not of
        the controller, so both sources compute the same answer and write the
        same value. No ownership handoff needed."""
        if not self._enabled or not sui.relay_installed():
            return False
        now = time.monotonic()
        if now - self._route_checked < self._ROUTE_POLL_S:
            return self._relay_on
        self._route_checked = now
        want = self.blocked(fast=True)
        if want and not self._relay_on:
            sui.relay_start()
        use = want and sui.relay_ready()
        if use != self._relay_on:
            self._relay_on = use
            sui.set_relay_active(use)
        return self._relay_on

    def poll_paused(self):
        """Cheap tick for the paused-for-Steam idle loop. Returns True while
        the escape reader should be open  the foreground is blocked, or was
        within the grace window. Costs one GetForegroundWindow while nothing
        changes (see blocked(fast=True)); the token probe only runs when the
        user actually switches window."""
        if not self._enabled:
            return False
        self.blocked(fast=True)
        return self.escape_armed()

    def disarm_escape(self):
        """Give up on the paused escape until the foreground changes again 
        for when the reader CAN'T open (Steam holds the Steam Controller
        exclusively, or none is attached). Without this the launcher would
        re-attempt the shared open every couple of seconds for the whole grace
        window, which is exactly the churn the grace window exists to avoid.
        A later switch into another administrator window re-arms it naturally,
        since blocked() stamps _last_blocked_at on the foreground change."""
        self._last_blocked_at = 0.0

    def escape_armed(self):
        """True while a blocked foreground is current or recent. Hysteresis on
        purpose: closing the shared HID the instant you alt-tab off Task
        Manager, then reopening when you go back, is exactly the churn we're
        avoiding."""
        if not self._enabled:
            return False
        if not self._last_blocked_at:
            return False
        return (self._blocked
                or (time.monotonic() - self._last_blocked_at)
                < self._ESCAPE_GRACE_S)

    def reset(self, sc=None):
        """Unconditional release  watcher teardown, mode switch, app exit.
        Injection must never stay gated behind a watcher that stopped
        delivering frames."""
        self._engaged = False
        self._hwnd = None
        self._checked = 0.0
        self._relay_on = False
        sui.set_relay_active(False)
        sui.set_suppressed(False)
        if sc is not None:
            try:
                sc.set_lizard(False)
            except Exception:
    ...
    def rescue(self):
        """The bindable "Escape Elevated Window" action: hand the foreground
        back to the last normal window we saw, which drops us out of the
        blocked state without ever touching the elevated window. Returns True
        if the foreground moved."""
        target = self._last_normal
        if not _hwnd_is_rescuable(target):
            target = _first_rescuable_window()
        if not target:
            return False
        ok = _force_foreground(target)
        if ok:
            # Re-probe on the next frame instead of waiting out the cache.
            self._hwnd = None
            self._checked = 0.0
        return ok
...
# --- Windows page: display scaling + accel-free mouse (scaling.ps1 port) ----
# Options → Windows → "Display Scaling": sets the Windows display scale
# factor via the registry (LogPixels + Win8DpiScaling) TOGETHER with the
# matching mouse settings so desktop mouse movement stays 1:1 with no
# acceleration at that scale ("MarkC fix" style). 100% turns "Enhance pointer
# precision" off outright with the stock curves; 125/150% counterintuitively
# keep it ON (MouseSpeed=1) but swap in a linear SmoothMouse curve pre-scaled
# for that DPI  the combination nets out to accel-free 1:1 movement. All
# values are HKCU (no admin needed, unlike the self-elevating .ps1  the
# elevation there was never actually required); state IS the registry  read
# live for the picker, nothing persisted to settings.json. The registry write
# alone would only land at the next sign-in, so Save ALSO pushes both halves
# to the running session (see _scaling_set_dpi_live / _scaling_reload_mouse)
# and the reboot is only a fallback when that push fails.

def _scaling_curve(rows):
    """A SmoothMouse curve REG_BINARY blob: 5 points x 8 bytes, each row's
    leading bytes given (the rest zero-padded)  matches the .reg hex lines."""
    return b"".join(bytes(r) + b"\x00" * (8 - len(r)) for r in rows)

# The Y curve is the Windows default and identical across all three presets.
_SCALING_YCURVE = _scaling_curve(
    [(), (0x00, 0x00, 0x38), (0x00, 0x00, 0x70), (0x00, 0x00, 0xA8),
     (0x00, 0x00, 0xE0)])
# choice -> (LogPixels DPI, (MouseSpeed, MouseThreshold1, MouseThreshold2),
#            EnablePerProcessSystemDPI, SmoothMouseXCurve)
_SCALING_CHOICES = {
    "100": (0x60, ("0", "0", "0"), 0, _scaling_curve(
        [(), (0xC0, 0xCC, 0x0C), (0x80, 0x99, 0x19), (0x40, 0x66, 0x26),
         (0x00, 0x33, 0x33)])),
    "125": (0x78, ("1", "6", "10"), 1, _scaling_curve(
        [(), (0x00, 0x00, 0x10), (0x00, 0x00, 0x20), (0x00, 0x00, 0x30),
         (0x00, 0x00, 0x40)])),
    "150": (0x90, ("1", "6", "10"), 1, _scaling_curve(
        [(), (0x30, 0x33, 0x13), (0x60, 0x66, 0x26), (0x90, 0x99, 0x39),
         (0xC0, 0xCC, 0x4C)])),
}


def scaling_get_choice():
    """Current Display Scaling notch ('100'/'125'/'150'), read live. The
    display-config API is asked first: on Windows 10 it is what the Settings
    slider drives and what scaling_apply now writes, whereas LogPixels is a
    legacy override that is routinely left at 96 on a machine actually running
    at 150%. It falls back to a forced LogPixels, then to the system DPI 
    which is itself only a hint here, since a DPI-unaware process (this one)
    is always told 96. Snaps to the nearest notch so scales this page can't
    express (110%, 175%) still land somewhere."""
    dpi = 0
    try:
        # First active source = the primary display in path order; a
        # multi-monitor setup with mismatched scales just shows that one.
        for adapter, source in _scaling_active_sources():
            rng = _scaling_dpi_range(adapter, source)
            if rng:
                dpi = round(rng[0] * 96 / 100)
                break
    except Exception:
        pass
    if dpi <= 0:
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER,
                ...
                dpi = int(winreg.QueryValueEx(k, "LogPixels")[0])
        except Exception:
            pass
    if dpi <= 0:
        try:
            ...
            dpi = 96
    return min(_SCALING_CHOICES,
               key=lambda c: abs(_SCALING_CHOICES[c][0] - dpi))


# -- applying the preset to the RUNNING session (no sign-out) ---------------
# LogPixels is only read at sign-in, but Windows' own Settings app changes the
# scale instantly  through an UNDOCUMENTED DisplayConfigSetDeviceInfo device-
# info type (GET/SET_DPI_SCALE, types -3/-4). A display's scale is stored as an
# index into a fixed ladder of percentages, expressed RELATIVE to that display's
# recommended rung, so applying 125% means "recommended index + n". Same call
# the Settings slider makes: per-monitor-aware apps rescale on the spot, the
# rest at their next launch, and nothing needs elevation.
_QDC_ONLY_ACTIVE_PATHS = 0x2
_DPI_SCALE_GET = -3
_DPI_SCALE_SET = -4
# The rungs the API indexes into (Windows' own list of allowed percentages).
_DPI_LADDER = (100, 125, 150, 175, 200, 225, 250, 300, 350, 400, 450, 500)
...
class _DC_DPI_GET(ctypes.Structure):
    """DISPLAYCONFIG_SOURCE_DPI_SCALE_GET  the three rungs are all offsets
    FROM the display's recommended scale (min is <= 0, max >= 0)."""
    _fields_ = [("header", _DC_HEADER), ("minScaleRel", ctypes.c_int32),
                ("curScaleRel", ctypes.c_int32), ("maxScaleRel", ctypes.c_int32)]


class _DC_DPI_SET(ctypes.Structure):
    _fields_ = [("header", _DC_HEADER), ("scaleRel", ctypes.c_int32)]
...
class _DC_MODE_INFO(ctypes.Structure):
    """DISPLAYCONFIG_MODE_INFO  opaque here; QueryDisplayConfig insists on a
    mode array but only the paths' source ids are needed."""
    _fields_ = [("blob", ctypes.c_ubyte * 64)]


def _scaling_active_sources():
    """[(adapterId, sourceId)] for every active display source, deduped 
    two cloned paths can share one source and must only be set once."""
    user32 = ctypes.windll.user32
    n_path, n_mode = ctypes.c_uint32(), ctypes.c_uint32()
    if user32.GetDisplayConfigBufferSizes(
            _QDC_ONLY_ACTIVE_PATHS, ctypes.byref(n_path),
            ctypes.byref(n_mode)) != 0:
        return []
    paths = (_DC_PATH_INFO * n_path.value)()
    modes = (_DC_MODE_INFO * n_mode.value)()
    if user32.QueryDisplayConfig(_QDC_ONLY_ACTIVE_PATHS, ctypes.byref(n_path),
                                 paths, ctypes.byref(n_mode), modes,
                                 None) != 0:
        return []
    out, seen = [], set()
    for p in paths[:n_path.value]:
        src = p.sourceInfo
        key = (src.adapterId.LowPart, src.adapterId.HighPart, src.id)
        if key not in seen:
            seen.add(key)
            out.append((src.adapterId, src.id))
    return out


def _scaling_dpi_range(adapter, source):
    """(current, recommended, minimum, maximum) scale percentages for one
    display source, or None when the query fails / reports rungs outside the
    known ladder (a driver we shouldn't be poking)."""
    pkt = _DC_DPI_GET()
    pkt.header.type = _DPI_SCALE_GET
    pkt.header.size = ctypes.sizeof(_DC_DPI_GET)
    pkt.header.adapterId = adapter
    pkt.header.id = source
    if ctypes.windll.user32.DisplayConfigGetDeviceInfo(ctypes.byref(pkt)) != 0:
        return None
    lo, hi = int(pkt.minScaleRel), int(pkt.maxScaleRel)
    # min is the offset of the FIRST rung, so |min| is the recommended rung's
    # own index; everything else keys off that.
    rec_idx = abs(lo)
    cur_idx = rec_idx + max(lo, min(hi, int(pkt.curScaleRel)))
    max_idx = rec_idx + hi
    if lo > 0 or hi < 0 or max_idx >= len(_DPI_LADDER):
        return None
    return (_DPI_LADDER[cur_idx], _DPI_LADDER[rec_idx], _DPI_LADDER[0],
            _DPI_LADDER[max_idx])


def _scaling_set_dpi_live(percent):
    """Set every active display to `percent` scaling right now (clamped to
    what each display allows). True when at least one display ended up at the
    requested scale  i.e. the Save visibly did something."""
    if os.name != "nt":
        return False
    ok = False
    for adapter, source in _scaling_active_sources():
        ...
        if rng is None:
            continue
        cur, rec, lo, hi = rng
        want = max(lo, min(hi, int(percent)))
        if want == cur:
            ok = True          # already there  nothing to push, still a win
            continue
        pkt = _DC_DPI_SET()
        pkt.header.type = _DPI_SCALE_SET
        pkt.header.size = ctypes.sizeof(_DC_DPI_SET)
        pkt.header.adapterId = adapter
        pkt.header.id = source
        pkt.scaleRel = _DPI_LADDER.index(want) - _DPI_LADDER.index(rec)
        if ctypes.windll.user32.DisplayConfigSetDeviceInfo(
                ctypes.byref(pkt)) == 0:
            ok = True
    return ok
...
_SPIF_UPDATEINIFILE = 0x1
_SPIF_SENDCHANGE = 0x2


def _scaling_reload_mouse(speed, t1, t2):
    """Make the just-written mouse half live. SystemParametersInfo pushes the
    acceleration flag + thresholds and the 1:1 sensitivity into the running
    session, but the ballistic CURVES (SmoothMouseXCurve/YCurve) are only read
    out of HKCU when per-user parameters are loaded  so finish with
    UpdatePerUserSystemParameters, the same reload cursor-scheme installers use
    to avoid a sign-out. True when the parameter push succeeded."""
    if os.name != "nt":
        return False
    user32 = ctypes.windll.user32
    flags = _SPIF_UPDATEINIFILE | _SPIF_SENDCHANGE
    ok = False
    try:
        # SPI_SETMOUSE takes {threshold1, threshold2, acceleration}.
        accel = (ctypes.c_int * 3)(int(t1), int(t2), int(speed))
        ok = bool(user32.SystemParametersInfoW(_SPI_SETMOUSE, 0, accel, flags))
        # 10 = the control panel's middle notch, the only 1:1 pointer speed
        # (passed BY VALUE in pvParam, hence the void* wrapper).
        user32.SystemParametersInfoW(_SPI_SETMOUSESPEED, 0,
                                     ctypes.c_void_p(10), flags)
    except Exception as e:
        print(f"scaling: mouse push failed: {e!r}")
    try:
        user32.UpdatePerUserSystemParameters(1, True)
    except Exception as e:
        print(f"scaling: per-user reload failed: {e!r}")
    return ok


def scaling_apply(choice):
    """Write one scaling.ps1 preset to HKCU (mouse + desktop keys) and push it
    to the running session. Returns "live" when the new scale took effect
    immediately, "reboot" when only the registry write landed (so a sign-in
    still owes the user the change), or False when nothing was written."""
    preset = _SCALING_CHOICES.get(str(choice))
    if preset is None:
        return False
    logpixels, (speed, t1, t2), per_proc, xcurve = preset
    try:
        import winreg
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER,
                              r"Control Panel\Mouse") as k:
            # 6-11 pointer speed: 10 = the control panel's default (middle)
            # notch, the only 1:1 one.
            winreg.SetValueEx(k, "MouseSensitivity", 0, winreg.REG_SZ, "10")
            winreg.SetValueEx(k, "MouseSpeed", 0, winreg.REG_SZ, speed)
            winreg.SetValueEx(k, "MouseThreshold1", 0, winreg.REG_SZ, t1)
            winreg.SetValueEx(k, "MouseThreshold2", 0, winreg.REG_SZ, t2)
            winreg.SetValueEx(k, "SmoothMouseXCurve", 0, winreg.REG_BINARY,
                              xcurve)
            winreg.SetValueEx(k, "SmoothMouseYCurve", 0, winreg.REG_BINARY,
                              _SCALING_YCURVE)
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER,
                              r"Control Panel\Desktop") as k:
            winreg.SetValueEx(k, "Win8DpiScaling", 0, winreg.REG_DWORD, 1)
            winreg.SetValueEx(k, "LogPixels", 0, winreg.REG_DWORD, logpixels)
            winreg.SetValueEx(k, "EnablePerProcessSystemDPI", 0,
                              winreg.REG_DWORD, per_proc)
    except Exception as e:
        print(f"scaling: write failed: {e!r}")
        return False
    # Registry is the durable copy; now hand the same preset to the live
    # session. The mouse half is reloaded either way (it costs nothing and
    # can't fail visibly)  only the DPI decides whether a sign-in is still
    # owed, since that is the part the user actually watches for.
    _scaling_reload_mouse(speed, t1, t2)
    live = _scaling_set_dpi_live(int(choice))
    print(f"scaling: {choice}% written ({'applied' if live else 'restart to apply'})")
    return "live" if live else "reboot"


# --- Windows page: "Loudness Equalization" audio enhancement ----------------
# Port of the user's "8 Sound Loudness EQ.ps1". Windows ships a built-in
# Loudness Equalization APO (audio processing object) that applies dynamic-range
# compression to playback  quiet passages are lifted and loud peaks are pulled
# down so overall level stays even. The switch lives in every ACTIVE render
# device's FxProperties under HKLM\...\MMDevices\Audio\Render\<guid>. Writing
# HKLM and restarting the audio service both need elevation, so the apply
# generates a .reg and imports it via `regedit /s` under the RunAs verb (one
# UAC prompt) then restarts audiosrv  the same path the script used. The keys
# below are the property-store values ("{GUID},PID") the Sound control panel
# itself writes. Windows-only; the whole block is guarded by os.name == "nt".
_LOUD_RENDER_BASE = (r"SOFTWARE\Microsoft\Windows\CurrentVersion"
                     r"\MMDevices\Audio\Render")
_LOUD_FLAG_KEY = "{fc52a749-4be9-4510-896e-966ba6525980},3"  # loudness on/off
# The full FxProperties block that turns the effect on: the pre/post/stream/
# mode effect CLSIDs that activate the enhancement APOs, the UI CLSID that
# surfaces it in the Sound panel, the loudness flag (bytes 8-9 = ff,ff → on)
# and the equalisation release time (max).
_LOUD_ENABLE_BODY = (
    ...
    '"{9c00eeed-edce-4cd8-ae08-cb05e8ef57a0},3"=hex:03,00,01,60,01,00,00,00,02,00,00,00\n'
)
# Disable = just clear the loudness flag value (leave the effect CLSIDs so any
# OTHER enhancement the user has on this device keeps working).
_LOUD_DISABLE_BODY = '"{fc52a749-4be9-4510-896e-966ba6525980},3"=-\n'


def _loudness_active_render_guids():
    """GUID subkeys of every ACTIVE playback device (MMDevices Render). The
    device's DeviceState value == 1 means active/enabled."""
    if os.name != "nt":
        return []
    import winreg
    out = []
    try:
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, _LOUD_RENDER_BASE) as root:
            i = 0
            while True:
                try:
                ...
                i += 1
                try:
                    with winreg.OpenKey(root, guid) as dk:
                        state = int(winreg.QueryValueEx(dk, "DeviceState")[0])
                except OSError:
                    continue
                if state == 1:
                    out.append(guid)
...
def loudness_eq_get():
    """True when Loudness Equalization is enabled on at least one active
    playback device  its FxProperties loudness flag carries the on marker
    (bytes 8-9 == 0xff,0xff). Read live; state IS the registry."""
    if os.name != "nt":
        return False
    import winreg
    for guid in _loudness_active_render_guids():
        try:
            with winreg.OpenKey(
                    winreg.HKEY_LOCAL_MACHINE,
                    _LOUD_RENDER_BASE + "\\" + guid + r"\FxProperties") as k:
                val = winreg.QueryValueEx(k, _LOUD_FLAG_KEY)[0]
        except OSError:
            continue
        if len(val) >= 10 and val[8] == 0xff and val[9] == 0xff:
            return True
    return False


def loudness_eq_apply(enabled):
    """Enable/disable Loudness Equalization on every active playback device by
    importing a generated .reg elevated (regedit /s under RunAs  one UAC
    prompt) and restarting the audio service so it takes effect. Returns True
    when the elevated helper was launched; the actual import happens after the
    user accepts UAC. No-op / False off Windows or with no active device."""
    if os.name != "nt":
        return False
    guids = _loudness_active_render_guids()
    if not guids:
        print("loudness: no active playback device")
        return False
    body = _LOUD_ENABLE_BODY if enabled else _LOUD_DISABLE_BODY
    lines = ["Windows Registry Editor Version 5.00", ""]
    for guid in guids:
        lines.append("[HKEY_LOCAL_MACHINE\\%s\\%s\\FxProperties]"
                     % (_LOUD_RENDER_BASE, guid))
        lines.append(body)
    content = "\r\n".join(lines) + "\r\n"
    try:
        import tempfile
        reg_path = os.path.join(tempfile.gettempdir(), "adusk_loudness.reg")
        # .reg "Version 5.00" files are UTF-16 LE with a BOM (utf-16 adds it).
        with open(reg_path, "w", encoding="utf-16") as f:
            f.write(content)
        # One elevated shell: import the reg silently, then bounce audiosrv
        # (-Force also restarts the services that depend on it).
        ps = ("regedit /s '%s'; Restart-Service audiosrv -Force"
              % reg_path.replace("'", "''"))
        rc = ctypes.windll.shell32.ShellExecuteW(
            None, "runas", "powershell.exe",
            '-NoProfile -WindowStyle Hidden -Command "%s"' % ps, None, 0)
        if int(rc) <= 32:
            print(f"loudness: elevation launch failed (rc={rc})")
            return False
        print("loudness: %s requested (elevated import + audiosrv restart)"
              % ("enable" if enabled else "disable"))
        return True
    except Exception as e:
        print(f"loudness: apply failed: {e!r}")
        return False


# --- Windows page: default playback (output) device -------------------------
# A dropdown that picks which device Windows plays audio out of. Enumeration +
# friendly names come from the registry (MMDevices Render, same base as the
# loudness helpers); the endpoint-ID subkey names ("{0.0.0.00000000}.{guid}")
# are exactly what the audio COM APIs use. Reading the current default needs
# IMMDeviceEnumerator (documented COM) and switching it needs IPolicyConfig
# (the undocumented endpoint-config interface the Sound panel uses)  both
# driven through raw ctypes vtable calls so no extra dependency is pulled in.
# Windows-only; every function no-ops off Windows.
...
# PKEY_Device_FriendlyName / PKEY_Device_DeviceDesc  the endpoint's display
# name ("Speakers (Realtek High Definition Audio)") and its short form.
_PKEY_FRIENDLY = "{a45c254e-df1c-4efd-8020-67d146a850e0},14"
_PKEY_DESC = "{a45c254e-df1c-4efd-8020-67d146a850e0},2"
# The MMDevices\Render subkeys are bare endpoint GUIDs, but the audio COM APIs
# (GetId / SetDefaultEndpoint) address them by the FULL endpoint-ID string:
# a fixed render-dataflow prefix + "." + that GUID. Capture would use
# "{0.0.1.00000000}."; we only touch render (playback).
...
def _com_call(ptr, index, argtypes, *args):
    """Invoke method #index on a COM interface pointer (raw vtable dispatch).
    Returns the HRESULT (c_long). `ptr` is the interface (c_void_p)."""
...
def _com_release(ptr):
    if ptr:
        _com_call(ptr, 2, [])   # IUnknown::Release


def playback_devices():
    """[(endpoint_id, friendly_name)] for every ACTIVE playback device. The ID
    is the full endpoint string (render prefix + the MMDevices Render subkey
    GUID) that IMMDevice::GetId returns and IPolicyConfig::SetDefaultEndpoint
    expects; the name comes from the device's property store in the registry."""
    if os.name != "nt":
        return []
    import winreg
    out = []
    for guid in _loudness_active_render_guids():
        ...
        try:
            with winreg.OpenKey(
                    winreg.HKEY_LOCAL_MACHINE,
                    _LOUD_RENDER_BASE + "\\" + guid + r"\Properties") as k:
...
def playback_get_default():
    """Endpoint ID of the current default playback device (eRender/eConsole),
    via IMMDeviceEnumerator. None on failure / off Windows."""
    if os.name != "nt":
        return None
    ole32 = ctypes.windll.ole32
    # Balance CoUninitialize only when WE actually initialized (S_OK/S_FALSE);
    # RPC_E_CHANGED_MODE means the thread already has a COM apartment.
    _hr_init = ole32.CoInitialize(None)
    _own_com = _hr_init in (0, 1)
    try:
        enum = ctypes.c_void_p()
        if ole32.CoCreateInstance(
                ctypes.byref(_guid(_CLSID_MMDeviceEnumerator)), None, 23,
                ctypes.byref(_guid(_IID_IMMDeviceEnumerator)),
                ctypes.byref(enum)) != 0 or not enum:
            return None
        dev = ctypes.c_void_p()
        # GetDefaultAudioEndpoint (vtbl 4): eRender=0, eConsole=0.
        hr = _com_call(enum, 4, [ctypes.c_int, ctypes.c_int,
                                 ctypes.POINTER(ctypes.c_void_p)],
                       0, 0, ctypes.byref(dev))
        result = None
        if hr == 0 and dev:
            pid = ctypes.c_void_p()
            # IMMDevice::GetId (vtbl 5) → COM-allocated LPWSTR.
            if _com_call(dev, 5, [ctypes.POINTER(ctypes.c_void_p)],
                         ctypes.byref(pid)) == 0 and pid:
                result = ctypes.cast(pid, ctypes.c_wchar_p).value
                ole32.CoTaskMemFree(pid)
            _com_release(dev)
        _com_release(enum)
        return result
    except Exception as e:
        print(f"playback: get default failed: {e!r}")
        return None
    finally:
        if _own_com:
...
def playback_set_default(endpoint_id):
    """Make endpoint_id the default playback device for all three roles
    (Console, Multimedia, Communications) via IPolicyConfig. True on success."""
    if os.name != "nt" or not endpoint_id:
        return False
    ole32 = ctypes.windll.ole32
    _hr_init = ole32.CoInitialize(None)
    _own_com = _hr_init in (0, 1)
    try:
        pc = ctypes.c_void_p()
        if ole32.CoCreateInstance(
                ctypes.byref(_guid(_CLSID_PolicyConfigClient)), None, 23,
                ctypes.byref(_guid(_IID_IPolicyConfig)),
                ctypes.byref(pc)) != 0 or not pc:
            print("playback: PolicyConfig unavailable")
            return False
        ok = True
        for role in (0, 1, 2):  # eConsole, eMultimedia, eCommunications
            # IPolicyConfig::SetDefaultEndpoint (vtbl 13).
            if _com_call(pc, 13, [ctypes.c_wchar_p, ctypes.c_int],
                         ctypes.c_wchar_p(endpoint_id), role) != 0:
                ok = False
        _com_release(pc)
        return ok
    except Exception as e:
        print(f"playback: set default failed: {e!r}")
        return False
    finally:
        if _own_com:
            ole32.CoUninitialize()
...
# Both bindable actions drive the WINDOWS DEFAULT capture device's endpoint
# mute  the exact same switch the Sound panel / volume mixer flips  so the
# effect is system-wide: every game, Discord, OBS, browser call and voice app
# sees the mic go silent at once, with no per-app hotkey to configure and no
# audio API hooking.
#
# Both the "Default Device" (eConsole) and the "Default Communication Device"
# (eCommunications) capture endpoints are muted together, deduped by endpoint
# id: Windows lets those be two DIFFERENT microphones, and a game using one
# while the toggle only touched the other would look broken.
#
# Everything runs on a worker thread with its own COM apartment (mic ops are
# queued, never coalesced  press/release ordering is what makes Push to Talk
# correct), so the controller input loop never blocks on COM. IAudioEndpointVolume
# is documented COM, driven through the same raw-ctypes vtable helpers as the
# playback-device dropdown above, so no new dependency is pulled in.
_IID_IAudioEndpointVolume = "{5CDF2C82-841E-4546-9722-0CF74078229A}"
_EDATAFLOW_CAPTURE = 1          # EDataFlow::eCapture
_MIC_ROLES = (0, 2)             # ERole::eConsole, ERole::eCommunications
_MIC_SETMUTE = 14               # IAudioEndpointVolume vtbl slots
_MIC_GETMUTE = 15

# Guards BOTH the PTT holder set and the op queue, so a press/release pair can
# never be enqueued out of order relative to the ref-count transition that
# produced it.
...
def _mic_capture_volumes(enum):
    """[IAudioEndpointVolume pointers] for the default capture endpoint of each
    role in _MIC_ROLES, deduped by endpoint id. Caller releases them."""
    ole32 = ctypes.windll.ole32
    out, seen = [], set()
    for role in _MIC_ROLES:
        dev = ctypes.c_void_p()
        # IMMDeviceEnumerator::GetDefaultAudioEndpoint (vtbl 4).
        if _com_call(enum, 4, [ctypes.c_int, ctypes.c_int,
                               ctypes.POINTER(ctypes.c_void_p)],
                     _EDATAFLOW_CAPTURE, role, ctypes.byref(dev)) != 0 or not dev:
            continue            # no mic at all, or none assigned to this role
        try:
            pid = ctypes.c_void_p()
            # IMMDevice::GetId (vtbl 5) → COM-allocated LPWSTR.
            if _com_call(dev, 5, [ctypes.POINTER(ctypes.c_void_p)],
                         ctypes.byref(pid)) == 0 and pid:
                did = ctypes.cast(pid, ctypes.c_wchar_p).value
                ole32.CoTaskMemFree(pid)
                if did in seen:
                    continue    # same physical mic serves both roles
                seen.add(did)
            vol = ctypes.c_void_p()
            # IMMDevice::Activate (vtbl 3): iid, CLSCTX_ALL, params, out.
            if _com_call(dev, 3, [ctypes.POINTER(_GUID), ctypes.c_uint32,
                                  ctypes.c_void_p,
                                  ctypes.POINTER(ctypes.c_void_p)],
                         ctypes.byref(_guid(_IID_IAudioEndpointVolume)), 23,
                         None, ctypes.byref(vol)) == 0 and vol:
                out.append(vol)
        finally:
            _com_release(dev)
    return out
...
def _mic_do(op):
    """Apply one mic op  True = mute, False = unmute, "toggle" = flip the
    current state  to every default capture endpoint. Owns its COM lifetime,
    so it is safe to call from the worker OR from an at-exit handler. Returns
    the mute state applied, or None when there is no usable mic."""
    if os.name != "nt":
        return None
    ole32 = ctypes.windll.ole32
    _hr_init = ole32.CoInitialize(None)
    ...
    try:
        ...
        vols = []
        try:
            vols = _mic_capture_volumes(enum)
            if not vols:
                return None     # no microphone on this machine
            if op == "toggle":
                # Read the primary (eConsole) endpoint and invert it; an
                # unreadable state falls back to muting (the safe direction).
                want = True
                m = ctypes.c_int()
                if _com_call(vols[0], _MIC_GETMUTE,
                             [ctypes.POINTER(ctypes.c_int)],
                             ctypes.byref(m)) == 0:
                    want = not bool(m.value)
            else:
                want = bool(op)
            for v in vols:
                # IAudioEndpointVolume::SetMute(BOOL, LPCGUID eventContext).
                _com_call(v, _MIC_SETMUTE, [ctypes.c_int, ctypes.c_void_p],
                          1 if want else 0, None)
            return want
        finally:
            for v in vols:
                _com_release(v)
...
def _mic_request(op):
    """Queue a mic op (called from the input loop  returns immediately).
    MUST be called with _mic_lock held so ordering matches the caller's state
    transition."""
    global _mic_thread
...
# --- Windows page: "Windows Resolution" (resolution + refresh dropdown) ------
# Lets people set their monitor's NATIVE resolution and refresh rate for the
# Windows desktop (a high-refresh monitor left at 60 Hz is the most common
# miss). Built to be safe on unknown hardware: the dropdown only ever lists
# modes the primary display's own driver reports (EnumDisplaySettings), every
# switch is validated with CDS_TEST before it is committed, and after a real
# change a native 15-second keep/revert popup restores the previous mode
# automatically on timeout  so even a mode the panel can't actually show
# (black screen, "no signal") heals itself with zero user input, exactly like
# Windows' own display settings dialog. State is read live; nothing is
# persisted to settings.json. Windows-only; helpers no-op elsewhere.

_DM_PELSWIDTH = 0x80000
_DM_PELSHEIGHT = 0x100000
_DM_DISPLAYFREQUENCY = 0x400000
_ENUM_CURRENT_SETTINGS = -1
_CDS_UPDATEREGISTRY = 0x1
_CDS_TEST = 0x2
...
def display_mode_label(mode):
    return "%d x %d @ %d Hz" % mode


def display_get_current():
    """(width, height, hz) active on the primary display, or None."""
    if os.name != "nt":
        return None
    dm = _DEVMODEW()
...
def display_modes():
    """[(w, h, hz)]  every 32-bit mode the primary display's driver reports,
    deduped and sorted best-first (largest resolution, then highest refresh),
    so the native pick is the top row. Tiny (sub-VGA) and interlaced-remnant
    (<24 Hz) modes are dropped; everything else stays, 30 Hz TVs included."""
    if os.name != "nt":
        return []
    user32 = ctypes.windll.user32
    seen, out = set(), []
    ...
    while True:
        ...
        if not user32.EnumDisplaySettingsW(None, i, ctypes.byref(dm)):
            break
        i += 1
        if int(dm.dmBitsPerPel) != 32:
            continue
        mode = (int(dm.dmPelsWidth), int(dm.dmPelsHeight),
                int(dm.dmDisplayFrequency))
        if mode[0] < 640 or mode[1] < 480 or mode[2] < 24 or mode in seen:
            continue
        seen.add(mode)
        out.append(mode)
    out.sort(key=lambda m: (-m[0] * m[1], -m[0], -m[2]))
    return out


def display_apply_mode(mode):
    """Switch the primary display to (w, h, hz). CDS_TEST first  a mode the
    driver rejects changes nothing  then CDS_UPDATEREGISTRY so the choice
    persists across reboots (same as the Windows Settings app). True on
    success."""
    if os.name != "nt":
        return False
    user32 = ctypes.windll.user32
    dm = _DEVMODEW()
    dm.dmSize = ctypes.sizeof(_DEVMODEW)
    # Seed from the live mode so only the fields we set below change.
    if not user32.EnumDisplaySettingsW(None, _ENUM_CURRENT_SETTINGS,
                                       ctypes.byref(dm)):
        return False
    dm.dmPelsWidth, dm.dmPelsHeight, dm.dmDisplayFrequency = mode
    dm.dmFields = _DM_PELSWIDTH | _DM_PELSHEIGHT | _DM_DISPLAYFREQUENCY
    if user32.ChangeDisplaySettingsW(ctypes.byref(dm), _CDS_TEST) \
            != _DISP_CHANGE_SUCCESSFUL:
        return False
    return user32.ChangeDisplaySettingsW(
        ctypes.byref(dm), _CDS_UPDATEREGISTRY) == _DISP_CHANGE_SUCCESSFUL


_CONFIRM_TITLE = "SteamlessInput  Windows Resolution"


def _display_esc_watcher(stop):
    """Background poll (runs beside the confirm box): the moment Esc or F8 is
    pressed anywhere  even with the box unfocused or the screen blank  dismiss
    the Yes/No box AS IF "No" were clicked, so the revert happens immediately
    instead of waiting out the 15-second timeout. Esc + F8 mirror CRU's
    restart64 revert keys. GetAsyncKeyState reads the global key state, so no
    keyboard focus is needed; the box is dismissed by posting a No-button click
    to it (WM_COMMAND / IDNO  a plain Yes/No box ignores WM_CLOSE, but honours
    a button command). Windows-only; harmless if the box already closed."""
    if os.name != "nt":
        return
    VK_ESCAPE, VK_F8, IDNO, WM_COMMAND = 0x1B, 0x77, 7, 0x0111
    user32 = ctypes.windll.user32
    # 0x8000 = key is down right now; 0x0001 = it was pressed since our last
    # call. Checking both catches a quick tap that starts and ends between two
    # polls, not just a key still held down. Prime the "since last call" bit
    # once up front so a stale pre-existing press doesn't fire instantly.
    user32.GetAsyncKeyState(VK_ESCAPE)
    user32.GetAsyncKeyState(VK_F8)
    while not stop.wait(0.03):
        esc = user32.GetAsyncKeyState(VK_ESCAPE)
        f8 = user32.GetAsyncKeyState(VK_F8)
        if (esc & 0x8001) or (f8 & 0x8001):
            hwnd = user32.FindWindowW("#32770", _CONFIRM_TITLE)
            if hwnd:
                user32.PostMessageW(hwnd, WM_COMMAND, IDNO, 0)
            return


def _display_confirm_or_revert(prev, new, notify):
    """Post-switch failsafe (runs on a worker thread so the picker's Tk thread
    never blocks): a native topmost Yes/No box with a 15-second timeout.
    Anything except an explicit Yes  No, Esc/F8, closing the box, the timeout,
    or a screen the user can't even see  puts the previous mode back. Uses
    MessageBoxTimeoutW (user32; native, thread-safe, no Tk involved); a side
    watcher thread turns a global Esc/F8 press into an immediate revert."""
    IDYES = 6
    stop = threading.Event()
    watcher = threading.Thread(target=_display_esc_watcher, args=(stop,),
                               daemon=True)
    watcher.start()
    try:
        ...
        res = ctypes.windll.user32.MessageBoxTimeoutW(
            None,
            "Keep this resolution?\n\n    %s\n\nReverting automatically in "
            "15 seconds  press Esc (or F8) to revert now, or if the screen "
            "went blank just wait." % display_mode_label(new),
            _CONFIRM_TITLE,
            0x00000004 | 0x00000020 | 0x00010000 | 0x00040000,
            0, 15000)
    except Exception as e:
        print(f"resolution: confirm box failed: {e!r}")
        res = 0
    finally:
        stop.set()
    if res == IDYES:
        notify("Windows Resolution",
               "Switched to %s." % display_mode_label(new))
        return
    if prev is not None and display_apply_mode(prev):
        notify("Windows Resolution",
               "Reverted to %s." % display_mode_label(prev))
    else:
        notify("Windows Resolution",
               "Could not revert  use Windows display settings if the "
               "resolution looks wrong.")


# --- ViGEmBus presence (tray "Game Mode" gate) ------------------------------
...
# and ViGEmClient.dll ships with us so nothing else notices it's missing.
# Cached for the process by gamepad.vigem_bus_available() itself.
def _vigem_bus_ok():
...
def _split_launch_args(args):
    """Split a user-typed argument string into argv tokens, Windows-style: a
    `"..."` run is one token with the quotes stripped, everything else splits
    on whitespace. No backslash-escape interpretation  paths like
    "C:\\Users\\me\\file.txt" must survive untouched, unlike POSIX shlex."""
...
def _launch_program(path, args=""):
    """Launch a program/script for a user-configured chord. With args, split
    them Windows-style (_split_launch_args) and Popen path+argv directly, no
    shell  a stray quote/`&`/`|` in a saved args string must never reach
    cmd.exe. Without args, use ShellExecute (os.startfile) so any file/URL/
    folder opens with its default handler. Non-blocking; runs on the HID read
    thread, so any failure is caught and logged, never raised.
    ...
    nothing like the one it was authored on:
      * VMENU_LAUNCH_DEFAULT_BROWSER  whatever browser is ACTUALLY the OS
        ...
        site.
      * VMENU_LAUNCH_STEAM  Steam's real install path from the registry.
        This one KEEPS its args ("-bigpicture"), so it falls through to the
        normal launch below instead of returning early."""
    try:
        path = (path or "").strip()
        ...
        if args.strip():
            import subprocess
            subprocess.Popen([path] + _split_launch_args(args))
        else:
...
def _run_user_script(script):
    ...
    PowerShell *or* batch  people paste old .bat snippets ("@echo off /
    shutdown /h / exit") as readily as PowerShell, and there is no
    interpreter dropdown, so vmenu_script_is_batch() infers which one from
    the syntax and this picks the matching shell and file extension.

    Either way the script goes to a temp file run by path rather than passed
    as a command-line string: a pasted script is multi-line and can contain
    any quoting, which a single command line mangles. For PowerShell,
    -NoProfile keeps it independent of the user's profile (and skips loading
    it, so the button reacts immediately), and -ExecutionPolicy Bypass is
    needed because the default machine policy blocks running a .ps1 from
    disk  this is the user's own pasted script, launched by their own button
    press.

    The temp file is cleaned up by a watcher thread once the script exits;
    it can't be deleted upfront because the shell needs to read it, and the
    script may run for a while."""
    try:
        ...
        import tempfile
        import threading
        is_batch = keybinds_runtime.vmenu_script_is_batch(script)
        if is_batch:
            # No BOM for a .bat: cmd.exe echoes the BOM bytes as a stray
            # command instead of skipping them. "mbcs" is the ANSI codepage
            # cmd itself reads a script in; anything unmappable degrades to
            # "?" rather than failing the whole write.
            suffix, encoding, errors = ".bat", "mbcs", "replace"
        else:
            # UTF-8 BOM: how PowerShell 5.1 (the Windows-bundled one) reliably
            # detects a UTF-8 .ps1  without it, non-ASCII in a pasted script
            # is misread as the legacy ANSI codepage.
            suffix, encoding, errors = ".ps1", "utf-8-sig", "strict"
        fd, path = tempfile.mkstemp(suffix=suffix, prefix="slinput_")
        try:
...
class _Watcher:
    ...
    def _fire_guide_action(self, sc, action, mode="pc"):
        ...
        if typ == "tap":
            ...
            if action[1] == keybinds_runtime.MIC_PTT_KEY:
                # Push to Talk reached an EDGE-only dispatch site (a stick zone,
                # a Steam/"..." tap) where there is no button-up to close on.
                # Degrade to a latch: fire once to open the mic, again to close.
                mic_ptt_hold("latch", "latch" not in _mic_ptt_holders)
            elif action[1] == keybinds_runtime.GYRO_MOUSE_KEY:
                # Same story for "Gyro To Mouse"  no release to end a hold on,
                # so the press flips the gyro instead (see gyro_action_flip).
            ...
            # Explorer and Windows Settings/Control Panel. Raw mouse_event, so
            # it needs the UIPI gate checked by hand (sui.Mouse can't emit
            # X-buttons, so this path doesn't go through the wrappers).
            try:
                if not sui.suppressed():
                    u = ctypes.windll.user32
        ...
        elif typ == "escape_elevated":
            # Hand the foreground back to the last non-elevated window, which
            # is the one-button way out of a window that has UIPI-blocked all
            # our input (Task Manager being the usual one). Works BECAUSE it
            # never touches the elevated window  only our own process and the
            # target  so there's nothing for UIPI to refuse. Still dispatched
            # while the guard is engaged: it doesn't inject.
            if not _UIPI.rescue():
                print("escape_elevated: no window to hand the foreground to")
        elif typ == "lock_pc":
    ...
    def _emit_scroll(self, laptop):
        ...
        same overall rate."""
        # UIPI gate: laptop mode's raw mouse_event bypasses the sui.Mouse
        # wrapper, so check it here. Whole-notch mode goes through sui.Mouse
        # and would be gated anyway. The accumulator is DROPPED rather than
        # held: keeping it would flush a lump of scroll the moment the guard
        # disengages, seconds after the user stopped moving.
        if sui.suppressed():
    ...
    def _handle_pad_wheel(self, sc, sci, now, smooth):
        ...
        if smooth:
            ...
            if units:
                ...
                # UIPI gate  raw mouse_event, outside the sui.Mouse wrapper.
                if not sui.suppressed():
                    ctypes.windll.user32.mouse_event(
                        0x0800, 0, 0, units * inv, 0)
            return
    ...
    def _handle_virtual_menu(self, sc, sci, now):
        ...
        # BUTTON trigger's is the button itself. _vmenu_hold_bit may be TWO
        # bits ORed together (an optional second combo button  see
        # _vmenu_rebuild_triggers), so "held" means ALL of them are down, not
        # just any one  `& mask == mask`, not the old truthy `&` test.
    ...
    def on_input(self, sc, sci):
        ...
        if self._should_abort():
            ...
            self._chord.release_all_held()
            # Same reasoning for the UIPI gate: a watcher that stops delivering
            # frames can never un-gate itself, so release it here.
            _UIPI.reset(sc)
            # ...and for the Virtual Menu claim: a watcher that stops running
            # would otherwise leave the config GUI's navigation dead for good.
            sc_viewer.set_vmenu_open(False)
            sc.addExit()
            return

        # Elevated-window (UIPI) fallback  DESKTOP mode only. Gamepad mode is
        # immune: the ViGEm pad is a real virtual HID device, so its reports go
        # through the driver stack like any physical pad and an elevated window
        # takes them fine. Desktop mode is the one that injects, so it's the
        # one that dies over Task Manager. While engaged, _UipiGuard hands the
        # cursor to firmware lizard mode and gates our injection (see
        # uinput._suppressed)  the dispatch below still RUNS, so the actions
        # that don't inject (Escape Elevated Window, Lock PC, Toggle Config
        # GUI, profile cycling, ...) keep working; only keystrokes, clicks and
        # cursor motion go quiet, and those were being discarded anyway.
        if self._gamepad is None:
            _UIPI.update(sc, self._chord, self._kind)
        elif _UIPI.engaged:
            _UIPI.reset(sc)
        ...
        if int(SCButtons.VIEW) not in self._guide_bind_bits:
            if guide_now and view_now and not self._chord.view_was_pressed:
                if _UIPI.engaged:
                    # Same chord, same MEANING ("put another window in front"),
                    # different route: Alt+Tab is injected, so an admin window
                    # would swallow it along with everything else. The rescue
                    # hands the foreground over directly instead, touching only
                    # us and the target  nothing for Windows to refuse. So the
                    # way out of a blocked window is the switch-window chord the
                    # user already has in their fingers, with nothing to bind.
                    _UIPI.rescue()
                else:
                    if not self._chord.alt_held:
                        self._chord.kb.pressEvent([sui.Keys.KEY_LEFTALT])
...
class _SdlDesktopController:
    ...
    def __init__(self, force_kill=None, binds=None, chords=None,
        ...
        # Guide+View escape edge (administrator-window rescue  see update()).
        self._uipi_view_was = False
    ...
    def _fire_action(self, action, kind=None, mode="pc"):
        ...
        if typ == "tap":
            ...
            # Raw mouse_event  needs the UIPI gate checked by hand (see the
            # _Watcher copy of this branch).
            try:
                if not sui.suppressed():
                    u = ctypes.windll.user32
                    u.mouse_event(0x0080, 0, 0, action[1], 0)  # MOUSEEVENTF_XDOWN
            ...
            # See the _Watcher copy  foreground handback, no injection.
            _UIPI.rescue()
        elif typ == "lock_pc":
            try:
                ctypes.windll.user32.LockWorkStation()
    ...
    def update(self, sci, now):
        b = sci.buttons
        # Elevated-window routing. These pads have no lizard mode, so the
        # uiAccess relay is their ONLY way to keep working on an administrator
        # window  and without this call they'd never be switched onto it.
        # Rate-limited inside, and skipped entirely (one cached bool) when no
        # relay is installed, which is the common case.
        _UIPI.ensure_relay_route()
        ...
        if guide_held:
            ...
            # Guide + View → escape an administrator window. The SDL
            # counterpart of the Steam Controller's Steam + View, and these
            # pads need it MORE: they have no firmware lizard mode, so when
            # Windows blocks our injection this is their only way out  which
            # is exactly why it can't require binding.
            #
            # The block test is deliberately LAZY: nothing calls _UIPI.update()
            # for SDL pads (there's no lizard fallback to drive), so instead of
            # probing the foreground on every frame of a 250 Hz loop we probe
            # only on the rising edge of the chord itself. Cost while playing:
            # one bit test per frame. When the foreground ISN'T blocked the
            # chord falls through untouched to whatever the user bound it to.
            _esc_now = bool(b & SCButtons.VIEW)
            if (_esc_now and not self._uipi_view_was
                    and _UIPI.enabled and _UIPI.blocked()):
                if _UIPI.rescue():
                    # Consume View for this frame so the bound guide action
                    # doesn't also fire on the way out.
                    b &= ~int(SCButtons.VIEW)
                    sci = sci._replace(buttons=b)
            self._uipi_view_was = _esc_now
...
class App:
    def __init__(self):
        ...
        for _kind in pads.KINDS:
            ...
            # An un-customized kind gets ITS OWN catalog defaults rather than
            # falling through to the Steam Controller's map  the pads whose
            # hardware lacks ZL/ZR or stick clicks (the NSO retro controllers)
            # would otherwise come up with Shift/Enter/Caps on buttons that
            # physically cannot fire, i.e. an on-screen keyboard with no Enter.
            if _kind == "sc":
        ...
        # Elevated-window (UIPI) guard: pick up the persisted setting and give
        # it a way to toast. The toast fires ONCE per run, the first time the
        # fallback engages  enough to explain why the binds went quiet without
        # nagging on every trip to Task Manager.
        _UIPI.set_enabled(bool(self.settings.get("elevated_fallback", True)))
        _UIPI.set_notifier(self._notify)
        # Steam+View edge state for the paused-for-Steam escape (_paused_nav_cb).
        self._paused_view_was = False
        ...
        # While paused for Steam the launcher blocks on _launcher_wake instead of
        # polling. Wake it the instant the config GUI (keybinds picker) takes
        # focus and asks for controller navigation, so the passive nav reader
        # starts promptly rather than up to a full 5 s pause-wait later.
        sc_viewer.set_nav_hook(self._launcher_wake.set)
        self._kbd_open = False
        # True while _kbd_open AND the Steam Controller/Deck pad that ceded
        # its device to adusk for this OSK session was actually open when it
        # did so. The launcher thread's SteamController instance closes its
        # HID handle for the whole time adusk owns it (see the sc.run() ->
        # adusk_app.main() handoff below), so _controller_connected() can't
        # rely on _current_sc during that window  this remembers the real
        # connection state across the handoff so the tray icon doesn't flash
        # "nocontroller" every time the keyboard opens.
        self._sc_held_for_kbd = False
        ...
        # virtual XInput device down and building a new one on reconnect is
        # what makes games lose the controller for good, so instead we PARK it:
        ...
        # Let the module-level bindable "Gyro To Mouse" action (buttons bound on
        # the Desktop / Chords / Gamepad tabs) flip through the SAME helper the
        ...
        # LIVE count of SDL pads currently connected (Switch/Xbox/PS/…),
        # republished by sdl_gamepad_thread each poll. Drives _controller_connected
        # (the tray icon state) alongside the HID SC/Deck link (_current_sc).
        self._sdl_pad_count = 0
        # Tray icon state machine (see _compute_tray_icon_state /
        # tray_icon_thread). _tray_icon_state is the name last pushed to the
        # tray; _tray_icon_cache memoizes the rendered PIL image per state.
        self._tray_icon_state = None
        self._tray_icon_cache = {}
        # ...and the same idea for the MENU: the last _menu_state() the popup
        # was built for, so the state-driven items (Game Mode's tick, the
        # Virtual Menu submenu) are only rebuilt when they'd actually differ.
        # None = never built, so the first tick always syncs.
        self._tray_menu_state = None
        self._was_charging = False
        # Options → Big Picture automation engine (big_picture.py): watches
        # for Steam's Big Picture window and applies/reverts the configured
        # display/audio/HDR/night-light/... changes, plus the controller
        # connect auto-open. Started from setup() (its crash-recovery pass
        # can touch the display, which must not delay __init__).
        self._bp_engine = big_picture.BigPictureEngine(
            settings=self.settings,
            save_settings=_save_settings,
            get_default_audio=playback_get_default,
            set_default_audio=playback_set_default,
            # Physical presence, not "are we driving it"  see the docstring;
            # this stays truthful while paused for Steam, which is exactly
            # when the auto-open/auto-close edges have to work.
            controller_connected=self._bp_controller_present,
            # Still tell the engine when we're paused: the presence SOURCE
            # swaps at the pause boundary (passive probe <-> live handle), and
            # the rebuild after Steam exits would otherwise look like a fresh
            # connect and re-launch Big Picture  restarting Steam, pausing us
            # again, forever.
    ...
    def _controller_connected(self):
        """True if any controller is live right now  the HID Steam Controller
        / Steam Deck pad (via sc.is_connected(), which also catches an
        unplugged dongle that never errors the blocking HID read  see its
        docstring) OR at least one SDL pad (Switch/Xbox/PS/…, counted by
        sdl_gamepad_thread)."""
        sc = self._current_sc
        if sc is not None and sc.is_connected():
            return True
        # The launcher thread closes its SteamController handle for the whole
        # time the OSK owns the device (see _sc_held_for_kbd)  that's not a
        # real disconnect, just a handoff, so don't report "nocontroller".
        if self._kbd_open and self._sc_held_for_kbd:
            return True
        return self._sdl_pad_count > 0

    # Passive HID presence cache for _bp_controller_present (hid.enumerate
    # costs ~25 ms, far too much to run every engine tick).
    _BP_HID_PROBE_TTL = 2.0

    def _bp_controller_present(self):
        """Is a controller physically present right now  the signal Options →
        Big Picture needs, as opposed to "are WE currently driving one".

        _controller_connected() answers the latter, and while paused for Steam
        the answer is always False: the launcher cedes the pads and closes the
        Steam Controller's HID handle, so _current_sc is None no matter what is
        plugged in. That made the engine's "When Steam Is Running" auto-open
        and its auto-close both dead on arrival  each of them can only ever
        fire while Steam (and therefore the pause) is up.

        So while paused, fall back to a PASSIVE probe that never opens a
        handle: the live SDL pad count (kept fresh by sdl_gamepad_thread's
        paused branch) plus a rate-limited HID enumeration for the SC / Deck.
        The HID probe reports the RECEIVER, so a wireless controller that has
        gone to sleep still reads as present  that errs toward "connected",
        which is the safe direction: auto-close simply won't fire rather than
        yanking Big Picture out from under someone."""
        if not self._steam_active.is_set():
            return self._controller_connected()
        if self._sdl_pad_count > 0:
            return True                      # already known, no probe needed
        now = time.monotonic()
        cached = getattr(self, "_bp_hid_probe", None)
        if cached is None or now - cached[0] >= self._BP_HID_PROBE_TTL:
            try:
                present = bool(present_hid_kinds())
            except Exception:
                present = False
            cached = (now, present)
            self._bp_hid_probe = cached
        return cached[1]

    def _compute_tray_icon_state(self):
        """Which tray icon the current runtime state calls for:
          "nocontroller"  no controller connected, OR paused because the
                            "pause when Steam is open" option is on and Steam
                            is running (self._steam_active).
          "gamepad"       a controller is in gamepad mode.
          "default"       otherwise.
        """
        if self._steam_active.is_set() or not self._controller_connected():
            return "nocontroller"
    ...
    def _refresh_tray_icon(self):
        """Push the icon for the current state to the tray, but only when it
        actually changed (setting icon.icon re-issues a Shell_NotifyIcon)."""
        state = self._compute_tray_icon_state()
        if state == self._tray_icon_state:
            return
        icon = getattr(self, "_icon_ref", None)
        if icon is None:
            return
        image = self._tray_icon_cache.get(state)
        if image is None:
            try:
                image = _load_icon_image(state)
            except Exception as e:
                print(f"tray icon load failed for {state!r}: {e!r}")
                return
            self._tray_icon_cache[state] = image
        self._tray_icon_state = state
        try:
            icon.icon = image
        except Exception as e:
            print(f"tray icon update failed: {e!r}")
    ...
    def _menu_state(self):
        ...
        decide whether the HMENU needs rebuilding (see _refresh_menu_state)."""
    ...
    def _refresh_menu_state(self):
        ...
        pystray's Win32 backend builds the popup HMENU once, when the menu is
        set  not on each right-click  so a `checked` callback is only
        re-read when something calls update_menu(). The click handlers do that
        for their own change, but Game Mode is equally reachable from the
        picker, the Hotkey Gamepad/Desktop chord, the hold-Start gesture and
        the auto-switcher, and a menu can be closed with Escape or by editing
        the list. This is the catch-all for all of those: one tuple compare per
        tick, and a rebuild only when it actually moved."""
    ...
    def tray_icon_thread(self):
        """Keep the tray icon in sync with connection / gamepad-mode / Steam-
        pause state. Cheap 0.5 s poll of already-published flags; the icon is
        only re-pushed on an actual state change (see _refresh_tray_icon).
        The menu's own state-driven items ride the same tick."""
    ...
    def game_mode_available(self, item=None):
        """Visibility callback for the tray item. Hidden outright on a PC with
        no ViGEmBus driver: Gamepad Mode cannot be turned on there at all, and
        a tray checkbox that silently refuses is worse than no checkbox. The
        picker's own toggle stays visible-but-disabled instead, because it has
        room to explain why (_vigem_missing_dialog); a tray menu doesn't."""
    ...
    def toggle_game_mode(self, icon=None, item=None):
        ...
        # (the HMENU is only rebuilt on the tray-state tick), and toggling off
        # a stale tick would silently do the opposite of what was clicked.
    ...
    def _paused_nav_cb(self, sc, sci):
        """Callback for the passive SC reader that runs while paused for Steam.
        Publishes the raw frame to the picker's nav channel, and  while an
        administrator window has input blocked  watches for the Steam + View
        escape. Nothing else: no desktop injection, no ViGEm, no lizard-mode
        changes  so this never leaks input into the Steam game that owns the
        controller, and Steam Input keeps the device to itself.

        The escape is safe to fire from here precisely because it touches no
        hardware: it only hands the Windows foreground from the administrator
        window to the last ordinary one (see _UipiGuard.rescue), which is pure
        Win32. That means Steam Input users  paused, with Steam driving the
        Steam Controller  get the same one-chord way out of a frozen Task
        Manager that the takeover runtime has."""
        if sci.status != SCStatus.INPUT:
            return
        sc_viewer.publish(sci)
        # Edge-detected so holding the chord fires once. Evaluated only while
        # armed, so a normal paused session costs one bit test per frame.
        if _UIPI.escape_armed():
            chord = bool(sci.buttons & _GUIDE_BITS) and bool(
                sci.buttons & SCButtons.VIEW)
            if chord and not self._paused_view_was:
                if _UIPI.rescue():
                    print("paused escape: Steam+View left the admin window")
            self._paused_view_was = chord
        else:
            self._paused_view_was = False

    def _paused_reader_wanted(self):
        """Why the paused passive reader should be open: the picker wants
        controller nav, the guided tour is on screen, or an administrator
        window has input blocked and the Steam + View escape needs watching.
        ONE reader serves all three  a second reader on the same device is
        what the multi-controller HID filters exist to prevent.

        The tour is its own reason rather than riding on nav_claimed() because
        nav drops the moment the picker stops being foreground, and the tour
        deliberately teaches chords that hand the foreground elsewhere  the
        reader has to outlive that or the press is never seen."""
        return (sc_viewer.nav_claimed() or sc_viewer.tutorial_claimed()
                or _UIPI.escape_armed())

    def _run_paused_sc_nav(self):
        """Paused-for-Steam passive reader: a lightweight PASSIVE, shared tap on
        the Steam Controller / Deck while Steam owns it. passive=True skips the
        lizard-mode takeover/watchdog and exclusive=False shares the HID with
        Steam (read-only), so this drives no XInput, injects nothing and never
        touches lizard mode  Steam Input keeps the device.

        Two jobs (see _paused_reader_wanted): steering the config GUI, and
        watching for the Steam + View escape while an administrator window has
        input blocked. Blocks until neither is wanted, Steam exits, or
        shutdown; a monitor thread tears the reader out within ~150 ms of any
        of those. Costs CPU only while one of the two actually needs it."""
        kinds_present = tuple(k for k in present_hid_kinds()
                              if k in pads.HID_KINDS)
        if not kinds_present:
            # No Steam Controller / Deck attached  SDL pads (if any) steer the
            # picker from sdl_gamepad_thread instead. Nothing for us to read;
            # idle until the picker closes or Steam exits wakes us. The escape
            # is HID-specific, so with no Steam Controller present there is
            # nothing to arm  drop it rather than re-enumerate HID on a timer.
            if not sc_viewer.nav_claimed():
                _UIPI.disarm_escape()
            self._launcher_wait(5.0)
            return
        sc = SteamController(callback=self._paused_nav_cb,
                             passive=True, exclusive=False,
                             kinds=kinds_present)
        done = threading.Event()

        def _monitor():
            # sc.run()'s HID read has a 200 ms timeout, so addExit() breaks it out
            # within ~200 ms even while the controller is idle (no frames).
            while not done.wait(0.15):
                # Refresh the UIPI guard from the live foreground. sc.run() has
                # the launcher blocked, so this is the ONLY thing polling while
                # the reader is up  without it _blocked would stay stale at
                # True and the escape grace window could never expire, pinning
                # the shared HID open for the rest of the Steam session. Still
                # just one GetForegroundWindow per tick (fast path re-probes
                # only when the window actually changes), and only while the
                # reader is already open.
                _UIPI.poll_paused()
                if (self._stop_event.is_set()
                        or not self._steam_active.is_set()
                        or not self._paused_reader_wanted()):
                    sc.addExit()
                    return
        ...
        try:
            ...
            print(f"paused SC nav reader: {e!r}")
        finally:
            done.set()
            sc.addExit()
        if not sc.opened:
            # Open failed  Steam likely holds the controller exclusively (or it
            # vanished). Back off so the paused loop doesn't spin retrying the
            # open while the picker stays up. If the ESCAPE was the only reason
            # we're here, stop retrying entirely: a device we can't share is one
            # we can't watch for the chord, and re-arming happens on its own the
            # next time the user moves to another administrator window.
            if not sc_viewer.nav_claimed():
                _UIPI.disarm_escape()
            self._launcher_wait(2.0)
    ...
    # How long a vanished wireless controller's virtual XInput device is held
    # open for it. Covers a Nintendo Bluetooth dropout (the controller is
    # usually back within a few seconds) with room to spare, while staying
    # short enough that a genuinely-switched-off pad frees its device promptly.
    ...
    def _apply_nintendo_sdl_hints(self):
        ...
        which  with this app's automatic multiplayer  is two players holding
        one Joy-Con each. A lone Joy-Con is presented sideways unless told
    ...
    def _park_sdl_gamepad(self, jid, pad):
        """A controller disappeared: keep its virtual XInput device open and
        zeroed for _PAD_DROPOUT_GRACE, reserved for that same controller.
        ...
        later; if we destroyed the ViGEm device in between, the game saw its
        ...
        print(f"sdl pad {jid} dropped  holding its XInput slot for "
    ...
    def _claim_parked_gamepad(self, jid):
        ...
        if time.monotonic() > expiry:
            try:
                ...
                pass
            return None
        print(f"sdl pad {jid} reconnected  resumed its held XInput slot")
        return pad
    ...
    def sdl_gamepad_thread(self):
        ...
        while not self._stop_event.is_set():
            ...
            if self._steam_active.is_set():
                ...
                guide_x_prev = False
                # Poll the pad even while ceded, for two jobs:
                #  1. Home+B force-shutdown  its whole purpose is to kill a
                #     running (often Steam) game, exactly when we're ceded.
                #  2. Picker navigation  if the config GUI is open and foreground
                #     (nav_claimed), feed its nav channel so an SDL pad can still
                #     steer the UI. Nothing else is injected into the game (no
                #     desktop kb/mouse, no ViGEm)  just nav frames.
                # (Skip while the OSK is open so we don't double-poll the pad
                # against adusk, which owns it then.)
                # The guided tour counts as a third job, and unlike nav it does
                # NOT drop when the picker loses the foreground: the chords it
                # teaches (Alt-Tab above all) take the foreground away by
                # design, so gating its frames on nav_claimed would go quiet on
                # the very press it is waiting for.
                nav = sc_viewer.nav_claimed() or sc_viewer.tutorial_claimed()
                sci = None
                if not self._kbd_open:
                    try:
                        ...
                        sci = None
                    # Keep the live pad count truthful even while ceded: the
                    # source is polled right here, and Options → Big Picture
                    # needs real controller presence DURING the pause (its
                    # "When Steam Is Running" auto-open and its auto-close both
                    # only ever fire while Steam  hence the pause  is up).
                    self._sdl_pad_count = len(src._pad_kinds)
                if nav and sci is not None:
                ...
                # ~60 Hz while actively steering the picker so nav feels live;
                # otherwise a lazy 10 Hz edge-detect for the HELD Home+B chord,
                # which runs for entire gaming sessions  don't wake 60x/sec when
                # nothing is watching.
                self._stop_event.wait(0.016 if nav else 0.1)
                continue
            ...
            # Live SDL-pad count for the tray icon state (_controller_connected).
            # _pad_kinds is add-on-connect / pop-on-disconnect, and already
            # excludes the HID SC and our own ViGEm pad.
            self._sdl_pad_count = len(src._pad_kinds)
            ...
            if sci is not None:
                ...
                if _h_mask:
                    ...
                    frames = {j: f._replace(buttons=f.buttons & ~_h_mask)
                              for j, f in frames.items()}
            # "Gyro To Mouse" hotkey chords from the active kind. Handed to the
            # shared ref-counted helper (as the HID watcher's does  see
            # App.handle_gyro_toggle) so this chord and any button bound to the
            # "Gyro To Mouse" action are two holders of one thing: the mode
            # semantics and the once-per-press "toggle" latch live in
            # gyro_action_hold, and this frame-driven path can't write a release
            # over a hold the bound button is asserting.
            ...
            if sci is not None:
                ...
                if self._sdl_gamepads:
                    ...
                    # their XInput devices for the controllers rather than
    ...
    def exit_app(self, icon, item):
        ...
        try:
            ...
            pass
        # Big Picture engine: leaves the session's system changes for the
        # next launch's recovery pass (BP may still be open), but restores
        # the blanked system cursors  nothing else ever would.
        try:
    ...
    def _playback_general_settings(self):
        """Windows page → default-playback-device dropdown state. Builds unique
        display labels for every active output device, caches the label→
        endpoint-id map on self (the apply reads it back), and marks the label
        of the current system default. Empty/absent off Windows so the picker
        simply shows no options."""
        devs = playback_devices()
        labels, self._playback_map = [], {}
        seen = {}
        for eid, name in devs:
            label = name
            if label in seen:                 # disambiguate duplicate names
                seen[label] += 1
                label = "%s (%d)" % (name, seen[name])
            else:
                seen[label] = 1
            labels.append(label)
            self._playback_map[label] = eid
        cur_id = playback_get_default()
        cur_label = next((lbl for lbl, eid in self._playback_map.items()
                          if eid == cur_id), labels[0] if labels else "")
        return {"playback_devices": labels, "playback_device": cur_label}

    def _display_general_settings(self):
        """Windows page → "Windows Resolution" dropdown state. Builds the
        label list (native/best mode first) for the PRIMARY display, caches
        the label→(w, h, hz) map on self (the apply reads it back), and marks
        the currently active mode. Empty off Windows / on enumeration failure
        so the picker simply hides the row."""
        modes = display_modes()
        labels = [display_mode_label(m) for m in modes]
        self._display_mode_map = dict(zip(labels, modes))
        cur = display_get_current()
        cur_label = display_mode_label(cur) if cur else ""
        if cur is not None and cur_label not in self._display_mode_map:
            # Odd active mode the enumeration didn't list (some scaled or
            # custom modes)  show it anyway so the dropdown reads true.
            labels.insert(0, cur_label)
            self._display_mode_map[cur_label] = cur
        return {"display_modes": labels, "display_mode": cur_label}

    def _bp_general_settings(self):
        """Big Picture page values for the picker. Toggles + device choices
        from settings.json; the display/audio dropdowns are built fresh from
        live enumeration each open (label→id maps cached on self for the
        apply, like the Windows page's playback/display dropdowns). Devices
        that are attached-but-inactive stay listed  an HDMI TV that is off
        right now must still be selectable as the Big Picture target."""
        s = self.settings
        disp_labels, self._bp_display_map = [], {}
        seen = {}
        for path, name, active in big_picture.list_displays():
            label = name if name not in seen else "%s (%d)" % (name,
                                                               seen[name] + 1)
            seen[name] = seen.get(name, 0) + 1
            if not active:
                label += " (inactive)"
            disp_labels.append(label)
            self._bp_display_map[label] = path
        cur_disp = next((lbl for lbl, p in self._bp_display_map.items()
                         if p == s.get("bp_display_device")),
                        disp_labels[0] if disp_labels else "")
        au_labels, self._bp_audio_map = [], {}
        seen = {}
        for eid, name, active in big_picture.list_audio_devices():
            label = name if name not in seen else "%s (%d)" % (name,
                                                               seen[name] + 1)
            seen[name] = seen.get(name, 0) + 1
            if not active:
            ...
            self._bp_audio_map[label] = eid
        cur_audio = next((lbl for lbl, e in self._bp_audio_map.items()
                          if e == s.get("bp_audio_device")),
                         au_labels[0] if au_labels else "")
        return {
            "bp_auto_launch": s.get("bp_auto_launch", "off"),
            "bp_auto_close": bool(s.get("bp_auto_close")),
            "bp_display": bool(s.get("bp_display")),
            "bp_displays": disp_labels,
            "bp_display_device": cur_disp,
            "bp_audio": bool(s.get("bp_audio")),
            "bp_audio_devices": au_labels,
            "bp_audio_device": cur_audio,
            "bp_hdr": bool(s.get("bp_hdr")),
            "bp_hdr_supported": big_picture.hdr_supported(),
            "bp_nightlight": bool(s.get("bp_nightlight")),
            "bp_nightlight_supported": big_picture.nightlight_supported(),
            "bp_pause_media": bool(s.get("bp_pause_media")),
            "bp_hide_cursor": bool(s.get("bp_hide_cursor")),
        }
    ...
    def _general_settings(self):
        ...
        return {
            ...
            "elevated_fallback": self.settings.get("elevated_fallback", True),
            # Read-only status for the input-helper row (not a setting).
            ...
            **self._sleep_general_settings(),
            # Big Picture page  toggles from settings.json + live-enumerated
            # display/audio device dropdowns (see _bp_general_settings).
            **self._bp_general_settings(),
            # Windows page  Display Scaling (scaling.ps1 port). State IS the
            # registry, read live here; never persisted to settings.json.
            "win_scaling": scaling_get_choice(),
            # Windows page  Loudness Equalization (8 Sound Loudness EQ.ps1
            # port). State IS the MMDevices registry, read live; not persisted.
            "loudness_eq": loudness_eq_get(),
            # Windows page  default playback device dropdown. Labels + current
            # selection built here (and the label→endpoint-id map cached on
            # self for the apply); state IS the system default, read live.
            **self._playback_general_settings(),
            # Windows page  "Windows Resolution" dropdown (native resolution
            # + refresh rate). Labels + current mode built here (label→mode
            # map cached on self for the apply); state IS the live display
            # mode, read fresh each open, never persisted.
            **self._display_general_settings(),
            ...
            **{"kbd_gyro_" + b: self.settings.get("kbd_gyro_" + b, dv)
            ...
            # Nintendo Bluetooth dropout mitigations + Joy-Con presentation
            # (shown on whichever Nintendo controller page hosts them).
    ...
    def _kind_general_settings(self):
        ...
        for kind in pads.KINDS:
            if kind != "sc":
                ...
                if kind in pads.HID_KINDS:
                    # HID-takeover kinds (Steam Deck, 2015 Steam Controller)
                    # carry the SC's trackpad settings too: Right Trackpad
                    # Sensitivity + the two Touchpad Keyboard Input buttons.
    ...
    # config files/registry, the Windows power scheme, the DPI registry, the
    # live display + audio devices  or whose apply can be REFUSED outright
    # (Sleep Manager without admin rights, a monitor that rejects the mode,
    # Steam config files that aren't there). For all of these the picker must
    # read the truth BACK after applying instead of assuming its own value
    # took, or a failed apply leaves a control showing On while the machine is
    # still Off. Maps setting -> which live-read group answers it; every
    # "sleep_*" key routes to the sleep group. Settings absent from this map
    # are plain settings.json writes that cannot fail, so the picker's value
    # is already the truth and no read-back is needed.
    _READBACK_GROUPS = {
        ...
        "bp_display_device": "bp", "bp_audio_device": "bp",
        # NOT here on purpose: "loudness_eq". Its apply only LAUNCHES an
        # elevated helper (regedit + audiosrv restart) and returns before the
        # user has even answered the UAC prompt, so an immediate read would
        # report the pre-change registry and wrongly snap the toggle back. It
        # corrects itself on the next picker open, which re-reads it live.
    }
    ...
    # plus Loudness Equalization: excluding it above is only about the instant
    # after an apply, when the elevated helper hasn't run yet. Seconds later a
    # poll reads the real registry, which is exactly how that toggle catches up
    # once the user answers UAC  or changes it from Windows itself.
    _READBACK_POLL_GROUPS = dict(_READBACK_GROUPS, loudness_eq="loudness",
                                 **_READBACK_POLL_EXTRA)
    ...
    # the picker's Tk thread while the poll runs on its own worker, and both
    # rebuild the cached label->id maps (_playback_map, _display_mode_map, the
    # two Big Picture ones) that the applies read back  so a poll landing
    # mid-apply could hand an apply a half-built map. One App instance, so a
    # class-level lock is the instance lock; RLock because the readback path
    # is reached from inside the same guard.
    ...
    def _general_readback(self, keys, poll=False):
        ...
        switch into a powercfg/device enumeration."""
        ...
        for grp, ks in want.items():
            try:
                if grp == "steam":
                    ...
                    out["win_scaling"] = scaling_get_choice()
                elif grp == "loudness":
                    out["loudness_eq"] = bool(loudness_eq_get())
                elif grp == "gamepad":
                    ...
                    live = {"playback": self._playback_general_settings,
                            "display": self._display_general_settings,
                            "bp": self._bp_general_settings,
                            "sleep": self._sleep_general_settings}[grp]()
                    for k in ks:
                        if k in live:
                            out[k] = live[k]
            except Exception as e:
    ...
    def _apply_general_setting(self, setting, value):
        ...
        if setting == "__poll__":
            ...
            # queue behind a powercfg read). It comes round again in seconds.
    ...
    def _apply_general_setting_locked(self, setting, value):
        ...
        if setting == "gamepad_mode":
        ...
        elif setting == "win_scaling":
            # Windows page → "Display Scaling" notch (scaling.ps1 port): HKCU
            # registry write plus a live push to the running session, staged in
            # the picker and applied here on Save. Toast either way so a silent
            # Save isn't mistaken for a no-op  and say so when the live push
            # didn't take, because then a sign-in IS still required.
            res = scaling_apply(value)
            if res == "live":
                self._notify("Display Scaling",
                             "%s%% applied. Apps that were already open may "
                             "need a restart to redraw sharply." % value)
            elif res:
                self._notify("Display Scaling",
                             "%s%% written  restart Windows (or sign out "
                             "and back in) to apply." % value)
            else:
                self._notify("Display Scaling",
                             "Could not write the scaling settings.")
        elif setting == "loudness_eq":
            # Windows page → "Loudness Equalization" toggle (Sound EQ .ps1
            # port): staged in the picker, applied here on Save. Needs
            # elevation (HKLM write + audio-service restart) so it pops one
            # UAC prompt; the change lands once the user accepts it.
            if loudness_eq_apply(bool(value)):
                self._notify(
                    "Loudness Equalization",
                    ("Enabling" if value else "Disabling")
                    + "  accept the prompt to apply (the audio service "
                      "restarts briefly).")
            else:
                self._notify("Loudness Equalization",
                             "Could not change Loudness Equalization "
                             "(no active playback device?).")
        elif setting == "playback_device":
            # Windows page → default playback device dropdown. Map the chosen
            # label back to its endpoint ID (rebuild the map if it's stale)
            # and switch the system default. Save-gated like the rest of the
            # page (_LIVE_EXCLUDE).
            eid = (getattr(self, "_playback_map", None) or {}).get(value)
            if eid is None:
                self._playback_general_settings()  # refresh the map, then retry
                eid = (self._playback_map or {}).get(value)
            if eid and playback_set_default(eid):
                self._notify("Playback Device",
                             "Now playing audio from “%s”." % value)
            else:
                self._notify("Playback Device",
                             "Could not switch the playback device.")
        elif setting == "display_mode":
            # Windows page → "Windows Resolution" dropdown. Map the label back
            # to its (w, h, hz) tuple (rebuild the map if it's stale), no-op if
            # that mode is already active. The switch itself is CDS_TEST-
            # validated, and the keep/revert countdown runs on a worker thread
            # (this handler is on the picker's Tk thread, which must not block
            # for 15 s) so a bad pick always restores itself.
            mode = (getattr(self, "_display_mode_map", None) or {}).get(value)
            if mode is None:
                self._display_general_settings()   # refresh the map, retry
                mode = (self._display_mode_map or {}).get(value)
            prev = display_get_current()
            if mode is None:
                self._notify("Windows Resolution",
                             "Could not resolve “%s”." % value)
            elif prev == mode:
                pass                               # already active  nothing to do
            elif display_apply_mode(mode):
                threading.Thread(
                    target=_display_confirm_or_revert,
                    args=(prev, mode, self._notify), daemon=True).start()
            else:
                self._notify("Windows Resolution",
                             "Your display rejected %s  nothing was changed."
                             % value)
        elif setting in ("bp_auto_launch", "bp_auto_close", "bp_display",
                         "bp_audio", "bp_hdr", "bp_nightlight",
                         "bp_pause_media", "bp_hide_cursor"):
            # Big Picture page toggles/dropdowns: persist + poke the engine so
            # its monitor thread re-evaluates what to watch. Save-gated in the
            # picker (_LIVE_EXCLUDE); nothing fires until Big Picture actually
            # opens/closes (or a controller connects, for the auto-launch).
        ...
        elif setting == "bp_display_device":
            # Big Picture page → target display dropdown. Map the label back
            # to its DisplayConfig device path (rebuild the map if stale).
            path = (getattr(self, "_bp_display_map", None) or {}).get(value)
            if path is None:
                self._bp_general_settings()      # refresh the map, then retry
                path = (self._bp_display_map or {}).get(value)
            if path:
                self.settings["bp_display_device"] = path
                _save_settings(self.settings)
                self._bp_engine.refresh()
            else:
                self._notify("Big Picture",
                             "Could not resolve the display “%s”." % value)
        elif setting == "bp_audio_device":
            # Big Picture page → target audio device dropdown (label →
            # MMDevice endpoint id, same stale-map retry as playback_device).
            eid = (getattr(self, "_bp_audio_map", None) or {}).get(value)
            if eid is None:
                self._bp_general_settings()
                eid = (self._bp_audio_map or {}).get(value)
            if eid:
                self.settings["bp_audio_device"] = eid
                _save_settings(self.settings)
                self._bp_engine.refresh()
            else:
                self._notify("Big Picture",
                             "Could not resolve the audio device “%s”."
                             % value)
            ...
            _save_settings(self.settings)
        elif setting == "install_relay":
            # A button, not a setting: acts immediately rather than waiting for
            # Save. The UAC prompt is the user's consent; nothing is persisted.
            ok = install_relay_elevated()
            # The installer runs asynchronously behind the prompt, so re-scan
            # rather than assume  the next check picks up a fresh install
            # without restarting the app.
            sui.relay_forget_install()
            return ok
            ...
            # subsystem (SDL, ViGEm, hotkeys, elevated-window guard, ...)  a
            # fresh process reading a fresh file can't miss one. Deferred to a
            # background thread: this call itself runs ON the picker's Tk
            # thread, and exit_app() tears the picker down on that SAME thread
            # (keybinds_picker.shutdown() would deadlock waiting for itself).
            ...
            return True
        elif setting == "elevated_fallback":
            # Live: the guard reads its own flag every frame. Turning it OFF
            # mid-engagement releases on the next frame (lizard back off,
            # injection un-gated); with no controller attached there are no
            # frames, so reset() here covers that case too.
            self.settings["elevated_fallback"] = bool(value)
            _save_settings(self.settings)
            _UIPI.set_enabled(self.settings["elevated_fallback"])
            if not self.settings["elevated_fallback"] and _UIPI.engaged:
                _UIPI.reset(self._current_sc)
            ...
            # goes through the same trigger path as Win+Ctrl+O instead of the
            # input-ignoring preview path. Not persisted.
            if value:
            ...
            if just_started and exit_on_launch:
                ...
                if self._current_sc is not None:
                    try:
                        ...
                        pass
                # This path tears the app down WITHOUT going through
                # exit_app(), so anything session-global that we switched off
                # must be put back here too  above all the mouse cursor. A Big
                # Picture session with "Hide Mouse Cursor" on blanks every
                # system cursor via SetSystemCursor; if we exit without
                # restoring, the user is left with an invisible pointer with no
                # app running to fix it.
                try:
                    self._bp_engine.stop()
                except Exception:
                    pass
    ...
    def _launcher_loop(self):
        ...
        while not self._stop_event.is_set():
            ...
            if self._steam_active.is_set():
                self._close_persistent_gamepad()
                # Normally we idle here with the Steam Controller fully ceded to
                # Steam. But if the config GUI (keybinds picker) is open and
                # foreground it asks for controller navigation (nav_claimed) 
                # run a lightweight passive, shared reader so the SC can still
                # steer the picker without taking the device back from Steam or
                # injecting anything. It costs CPU only while the picker is
                # foreground; the instant it closes (or Steam exits) the reader
                # stops and we fall back to the blocking idle wait. Opening the
                # picker fires set_nav_hook -> _launcher_wake, so it starts the
                # reader promptly instead of up to 5 s later.
                # ...and the same reader carries the elevated-window escape, so
                # Steam Input users get a way out of a frozen administrator
                # window even though WE aren't driving the controller. The idle
                # tick that arms it is one GetForegroundWindow (poll_paused →
                # blocked(fast=True) re-probes only on a foreground CHANGE), and
                # the reader then stays open through a grace window so bouncing
                # in and out of Task Manager can't churn the shared HID open and
                # closed. Nothing here touches lizard mode or the device state
                # Steam owns.
                if self._paused_reader_wanted():
                    self._run_paused_sc_nav()
                elif _UIPI.poll_paused():
                    continue          # just armed  open the reader now
                else:
                    self._launcher_wait(2.0)
                continue
            ...
            self._current_sc = sc
            # Fresh reader = fresh UIPI state. A device that dropped WHILE the
            # elevated fallback was engaged never got its disengage frame, and
            # a stale gate would silence the new reader's injection forever.
            # The new SteamController already opens with lizard off, so this
            # only has to clear our side.
            _UIPI.reset()
            ...
            # the user's last app forward would bury the tour behind it (the
            # exact bug: the keyboard opened, Notepad came to the front, and the
            # tutorial only reappeared once the GUI was clicked again).
            ...
            self._sc_held_for_kbd = sc.opened
...
# Tray icon .ico per runtime state (see App._compute_tray_icon_state):
#   nocontroller  no controller connected, OR paused for Steam
#   gamepad       a controller is in gamepad mode
#   default       everything else
_TRAY_ICON_FILES = {
    "default": "tray_default.ico",
    "gamepad": "tray_gamepad.ico",
    "nocontroller": "tray_nocontroller.ico",
}


def _load_icon_image(name="default"):
    """Load the tray icon PIL image for a runtime state ("default" /
    "gamepad" / "nocontroller"). Prefers that state's own multi-resolution
    .ico (hand-tuned per size, so the small tray frame is crisp), then the
    generic app_icon.ico, then the in-OSK keyboard glyph PNG."""
    ...
    candidates = []
    fname = _TRAY_ICON_FILES.get(name)
    if fname:
        candidates.append(os.path.join(base, fname))
    candidates.append(os.path.join(base, "app_icon.ico"))
...
# first launch is the slowest possible build (nothing cached, Defender still
# scanning the freshly extracted onefile payload).
...
def _first_run_reveal(app):
    ...
    Runs on its own daemon thread  the wait must not block pystray's setup
    callback, which is still holding up the tray icon.
...
def main():
    app = App()
    image = _load_icon_image(app._compute_tray_icon_state())

    # A plain right-click tray menu  no custom positioning/owner-draw.
    # Left-click (the `default` item) opens the Manager, matching the previous
    # behavior. Every SETTING that used to live here moved into the Manager's
    # Options tab; what's left is the handful of things you reach for WITHOUT
    # wanting to open a window  typically mid-game, with a controller in hand.
    #   Keyboard      toggle the on-screen keyboard
    #   Manager       open the Keybinds manager (also the left-click default)
    #   Game Mode     force gamepad mode on/off (hidden with no ViGEmBus)
    #   Virtual Menu  show/hide one of the user's menus (hidden if none)
    #   Exit          quit
    # The two state-driven items are last, and the Win32 HMENU is only
    # rebuilt when pystray is told to (update_menu)  see _refresh_menu_state,
    # which keeps their tick marks honest against changes made anywhere else.
    menu = pystray.Menu(
        pystray.MenuItem("Keyboard", app.open_or_close_keyboard),
        pystray.MenuItem("Manager", app.open_keybinds, default=True),
    ...
    def setup(icon):
        ...
        threading.Thread(target=app.tray_icon_thread, daemon=True).start()
        threading.Thread(target=open_gui_watch_thread, daemon=True).start()
        # Big Picture engine: start() runs the crash-recovery pass (it can
        # re-apply a display topology) before spawning its monitor thread 
        # keep that off the tray thread.
        threading.Thread(target=app._bp_engine.start, daemon=True,
                         name="bp-engine-start").start()
