...
# Optional tray-side callback fired ONCE on the nav claim's rising edge
# (False->True), i.e. the instant the picker becomes foreground and wants
# controller navigation. The tray uses it to wake its (otherwise blocked)
# launcher out of the Steam-pause idle wait so the passive nav reader starts
# promptly instead of up to 5 s later. set_nav is called at ~30 Hz, so the
# hook MUST only fire on the transition, never per tick.
_on_nav_rise = None


def set_nav_hook(fn):
    """Register (or clear with None) the nav-rising-edge callback. Runs on the
    picker's Tk thread inside set_nav  keep it O(1) and non-blocking."""
    global _on_nav_rise
    _on_nav_rise = fn
...
def set_nav(on):
    ...
    global _nav
    on = bool(on)
    rose = on and not _nav
    _nav = on
    # Fire the wake hook only on the False->True edge so the tray can start its
    # paused-for-Steam nav reader the moment the picker takes focus.
    if rose and _on_nav_rise is not None:
        try:
            _on_nav_rise()
        except Exception:
            pass
