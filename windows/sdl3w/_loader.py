...
from source (windows/sdl3w/dll) and when frozen by PyInstaller (build.py sends
them to the bundle root, where PyInstaller's own dependency analysis already
puts SDL3.dll  see the note there).
...
def _candidate_dirs():
    ...
    dirs = []
    # PyInstaller onefile extracts to sys._MEIPASS; build.py drops the DLLs at
    # the bundle root. The sdl3w/dll subdir is still searched first so an older
    # bundle (or a hand-assembled one) laid out that way keeps working.
