...
class VBus:
    ...
    def __del__(self):
        # check_err above raises on a machine with no ViGEmBus driver, so this
        # object can be GC'd half-built. A __del__ that assumed a successful
        # __init__ then raised AttributeError on every collection  "Exception
        # ignored while calling deallocator", spraying crash.log on any PC
        # without the driver. Nothing to release if we never connected.
        if getattr(self, "_busp", None) is None:
            return
...
def get_vbus():
    ...
    installed. SteamlessInput only needs the bus once gamepad mode creates a
...
class VGamepad(ABC):
    def __init__(self):
        # Bound FIRST: get_vbus() raises without the ViGEmBus driver, and the
        # assert below can fail too, so __del__ must find these attributes
        # present-but-empty rather than missing (see VBus.__del__).
        self._busp = None
        self._devicep = None
        self.vbus = get_vbus()
    ...
    def __del__(self):
        if getattr(self, "_busp", None) is None \
                or getattr(self, "_devicep", None) is None:
            return
