...
# 30% more transparent, than the tuned "medium" baseline. Kept as the named
# reference notches; the slider interpolates a CONTINUOUS scale between them.
...
# The named notches as positions on the 0..1 transparency slider, in order.
_TRANSP_NOTCHES = [("off", 0.0), ("low", 1.0 / 3), ("medium", 2.0 / 3), ("high", 1.0)]
# Piecewise-linear control points (fraction, opacity scale) the slider follows.
# Off (0.0) disables transparency; the named notches use their exact tray scales;
# the 0+ boundary is anchored a touch MORE opaque than "low" so sliding up out of
# Off ramps smoothly into the most-opaque transparent look instead of jumping.
_TRANSP_POINTS = [(0.0, 1.6), (1.0 / 3, 1.3), (2.0 / 3, 1.0), (1.0, 0.7)]


def transparency_level_to_fraction(level):
    """Slider fraction (0..1) for a named tray level; unknown → 0.0 (off)."""
    return dict(_TRANSP_NOTCHES).get(level, 0.0)


def transparency_fraction_to_level(frac):
    """Nearest named tray level for a slider fraction (for menu checkmarks)."""
    return min(_TRANSP_NOTCHES, key=lambda nf: abs(nf[1] - frac))[0]


def _interp_scale(frac):
    """Piecewise-linear opacity scale at fraction `frac` over _TRANSP_POINTS.
    At a named-notch fraction it returns that notch's exact scale."""
    pts = _TRANSP_POINTS
    if frac <= pts[0][0]:
        return pts[0][1]
    if frac >= pts[-1][0]:
...
def set_transparency_fraction(frac):
    """Apply a CONTINUOUS transparency level from a 0..1 slider fraction. 0 = off
    (opaque); any value above 0 enables transparent mode with an interpolated
    opacity scale, so positions between the named notches give in-between opacity."""
    ...
    try:
        ...
        frac = 0.0
    on = frac > 1e-6
    scale = _interp_scale(frac) if on else 1.0
    with _active_lock:
...
def set_transparency(level):
    """Apply a transparency level by NAME ("off"/"low"/"medium"/"high"). Thin
    back-compat wrapper over the continuous fraction path (each name maps to its
    notch fraction, preserving the original exact scales)."""
    set_transparency_fraction(transparency_level_to_fraction(level))
