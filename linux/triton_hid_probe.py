"""Valve HID wire-format probe / firmware-diff tool.

Primarily for the Triton (Steam Controller 2026), but it walks every vendor
interface `steamcontroller` knows about, so it also captures a Steam Deck's
built-in pad and the ORIGINAL 2015 Steam Controller  useful for confirming
what those actually stream on hardware nobody here has (see the per-family
baselines printed into the report).
...
_LIZARD_REPORTS = {
    "sc": (( DISABLE_LIZARD_REPORT,), (ENABLE_LIZARD_REPORT,)),
    "sc2015": (SC2015_LIZARD_OFF_REPORTS, SC2015_LIZARD_ON_REPORTS),
    "steam_deck": (DECK_LIZARD_OFF_REPORTS, DECK_LIZARD_ON_REPORTS),
}


def _lizard_reports_for(kind, enabled):
    """The feature report(s) that turn firmware mouse/keyboard emulation on or
    off for one controller family  the probe has to send the RIGHT ones or the
    device keeps emitting boot-keyboard reports instead of game state."""
    off, on = _LIZARD_REPORTS.get(kind, _LIZARD_REPORTS["sc"])
...
def probe_interface(cand, seconds, out):
    ...
    try:
        ...
        # streams full game-input state reports on this interface. Each family
        # takes a different sequence (the 2015 controller and the Deck use
        # Valve's legacy report-id-0 framing, the Triton a settings write).
        for _report in _lizard_reports_for(cand.get("_kind", "sc"), False):
            try:
                dev.send_feature_report(_report)
            except Exception:
                pass
        ...
        try:
            for _report in _lizard_reports_for(
                    cand.get("_kind", "sc"), True):
                dev.send_feature_report(_report)
        except Exception:
            pass
        try:
...
def main():
    ...
    out.append("BASELINE (2015 Steam Controller, NOT yet hardware-confirmed): "
               "64-byte frames starting 01 00 01 3C; after that header  "
               "packetnum=bytes 4-7, buttons=8-10, triggers=11-12, "
               "lpad/stick=16-19, rpad=20-23, gyro=34-39. A ucType 4 status "
               "frame (01 00 04 ..) carries the battery voltage at 12-13. "
               "See _parse_sc2015 in steamcontroller/__init__.py.")
