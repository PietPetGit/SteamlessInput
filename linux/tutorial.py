...
class Tutorial:
    ...
    def _poll_foreground(self):
        ...
        No-op in practice on Linux: _app_is_active() is hardcoded True there
        (X11/Wayland focus isn't reachable the way GetForegroundWindow is 
        see the picker's own definition), so only the "is the manager itself on
        screen" half of the test does anything. Kept identical to the Windows
        tree so the two stay one file, and so a future real is-active answer
        only has to be implemented in that one place."""
        self._poll_aid = None
        if self._panel is None:
            return
